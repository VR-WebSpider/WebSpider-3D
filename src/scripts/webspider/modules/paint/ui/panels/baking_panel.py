...
def _draw_placeholder_ui(layout, context):
    ...
    col.label(text="  1. Select an object with a WebSpider 3D material")
...
def _draw_baked_channels_section(layout, mp, node):
    ...
    Args:
        layout: Blender UI layout
        mp: MPaint property group
        node: The WebSpider 3D node tree
...
def _draw_inline_material_library(layout, context):
    ...
    search_row.prop(webspider3d_ui, "material_library_search", text="", icon='VIEWZOOM')
    if webspider3d_ui.material_library_search:
    ...
    cat_row.menu("BAKING_MT_material_category", text=_get_category_display_name(webspider3d_ui.material_library_category))

    # === GET FILTERED MATERIALS ===
    try:
        category = webspider3d_ui.material_library_category
        search_text = webspider3d_ui.material_library_search.lower()
        ...
        prefs = get_webspider3d_paint_preferences()
        if prefs:
    ...
    def draw(self, context):
        ...
        for cat_id, cat_name in categories:
            op = layout.operator(
                ...
                text=cat_name,
                icon='CHECKMARK' if webspider3d_ui.material_library_category == cat_id else 'NONE'
            )
            op.category = cat_id
...
class BAKING_OT_set_material_category(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if hasattr(wm, 'webspider3d_ui'):
            wm.webspider3d_ui.material_library_category = self.category
        return {'FINISHED'}
...
class BAKING_PT_main(Panel):
    ...
    def draw(self, context):
        ...
        tab_row.scale_y = 1.5
        tab_row.prop(webspider3d_ui, "baking_panel_tab", expand=True)

        layout.separator()

        # ========== CONDITIONAL SECTIONS BASED ON TAB ==========
        if webspider3d_ui.baking_panel_tab == 'MESH_BAKING':
            # MESH BAKING TAB
        ...
        elif webspider3d_ui.baking_panel_tab == 'ASSETS':
            # ASSETS TAB
            # - Procedural Materials section

            _draw_procedural_materials_section(layout)

        elif webspider3d_ui.baking_panel_tab == 'TEXTURE_EXPORT':
            # TEXTURE EXPORT TAB
        ...
        elif webspider3d_ui.baking_panel_tab == 'ASSET_EXPORT':
            # ASSET EXPORT TAB
