...
{
  PropertyRNA *groups_prop = RNA_struct_find_property(ctx.scene_ptr, "webspider_ai_moodboard_groups");
  if (groups_prop) {
    PointerRNA group_ptr;
                                                      ...
                                                      const wmEvent *event)
{
  ...
  {
    wmOperatorType *ot = WM_operatortype_find("WEBSPIDER_AI_OT_moodboard_edit_textbox", false);
    if (ot) {
      PointerRNA ptr;
  ...
  if (clicked_index == -1) {
    ...
    wmOperatorType *ot = WM_operatortype_find("WEBSPIDER_AI_OT_moodboard_box_select", false);
    if (ot) {
      PointerRNA ptr;
      WM_operator_properties_create_ptr(&ptr, ot);
...
void WEBSPIDER_AI_OT_moodboard_select_image(wmOperatorType *ot)
{
  ...
  ot->idname = "WEBSPIDER_AI_OT_moodboard_select_image";
  ot->description = "Select and move an image on the moodboard";

  ot->invoke = blender::ed::webspider_ai::moodboard_select_image_invoke;
  ot->modal = blender::ed::webspider_ai::moodboard_select_image_modal;
  ot->cancel = blender::ed::webspider_ai::moodboard_select_image_cancel;
