 ...
 * every frame. Split from webspider_ai_chat_messages.cc for modularity.
 */
...
float webspider_ai_chat_build_layout_cache(SpaceWebSpider AIChat *swebspider_ai,
                                    Main *bmain,
                                    ...
                                    int msg_count)
{
  ...
  webspider_ai_chat_clear_layout_cache(swebspider_ai); /* Clear previous frame's cache */
      ...
      if (is_slot_msg && layout.has_feedback) {
        ...
        if (layout.feedback_comment_expanded) {
          layout.feedback_comment_input_height =
              webspider_ai_chat_feedback_comment_input_height(&msg_ptr, bubble_width);
