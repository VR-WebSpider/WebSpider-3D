...
def _redraw_webspider_chat_areas():
...
class WEBSPIDER_BYOK_OT_open_dialog(Operator):
    """Configure your own API provider and key for the WebSpider 3D agent"""
    bl_idname = "webspider3d_byok.open_dialog"
    bl_label = "AI Provider Settings"
    bl_description = (
        "Use your own API key for the WebSpider 3D agent. "
        "While active, WebSpider 3D credits are not charged for agent requests."
    )
    ...
    def draw(self, context):
        ...
        subtitle.label(text="Choose the provider and model the WebSpider 3D agent should use.")
        ...
        if state == 'SAVING':
            ...
            if wm.byok_is_active:
                ...
                remove_col.operator(
                    WEBSPIDER_BYOK_OT_request_remove.bl_idname,
                    text="Remove",
                    icon='TRASH',
                )
            actions.operator(
                WEBSPIDER_BYOK_OT_save.bl_idname,
                text="Save",
                icon='CHECKMARK',
            )
    ...
    def _draw_cloud_fields(self, box, col, wm):
        ...
        hint.enabled = False
        hint.label(
            text="Your API key is stored encrypted and used only for WebSpider 3D agent requests.",
            icon='INFO',
        )
    ...
    def _draw_codex_fields(self, box, col, wm):
        ...
        load_row.operator(
            WEBSPIDER_BYOK_OT_codex_load_file.bl_idname,
        ...
        paste_row.operator(WEBSPIDER_BYOK_OT_codex_paste.bl_idname, text="", icon='PASTEDOWN')
        ...
        for line in (
            ...
            "Uses your ChatGPT subscription — WebSpider 3D credits are not charged.",
        ):
            row = box.row()
            row.enabled = False
    ...
    def _draw_confirm_remove(self, layout):
        ...
        body.enabled = False
        body.label(text="The agent will use WebSpider 3D's default provider again.")
        body.label(text="WebSpider 3D credits will be charged for future agent requests.")

        layout.separator(factor=0.8)
        row = layout.row(align=True)
        row.operator(
            WEBSPIDER_BYOK_OT_cancel_remove.bl_idname,
            text="Cancel",
            icon='CANCEL',
        )
        ...
        confirm.alert = True
        confirm.operator(
            WEBSPIDER_BYOK_OT_confirm_remove.bl_idname,
            text="Confirm Remove",
            icon='TRASH',
        )
...
class WEBSPIDER_BYOK_OT_save(Operator):
    """Validate with the provider and save your API key"""
    bl_idname = "webspider3d_byok.save"
    bl_label = "Save"
    bl_options = {'INTERNAL'}
...
class WEBSPIDER_BYOK_OT_codex_load_file(Operator):
    """Read ~/.codex/auth.json from this machine into the field"""
    bl_idname = "webspider3d_byok.codex_load_file"
...
class WEBSPIDER_BYOK_OT_codex_paste(Operator):
    """Paste your ~/.codex/auth.json from the clipboard into the field"""
    bl_idname = "webspider3d_byok.codex_paste"
...
class WEBSPIDER_BYOK_OT_request_remove(Operator):
    """Start the remove-confirmation flow"""
    bl_idname = "webspider3d_byok.request_remove"
    bl_label = "Remove"
    bl_options = {'INTERNAL'}
...
class WEBSPIDER_BYOK_OT_cancel_remove(Operator):
    """Cancel the remove flow and return to the main dialog"""
    bl_idname = "webspider3d_byok.cancel_remove"
    bl_label = "Cancel"
    bl_options = {'INTERNAL'}
...
class WEBSPIDER_BYOK_OT_confirm_remove(Operator):
    """Delete the stored API key server-side"""
    bl_idname = "webspider3d_byok.confirm_remove"
...
class WEBSPIDER_BYOK_OT_fetch_state(Operator):
    """Refresh cached BYOK state from the server (non-interactive)"""
    bl_idname = "webspider3d_byok.fetch_state"
...
def _on_fetch_done(success: bool, data, err):
    ...
    subsequent agent calls use WebSpider 3D's system keys.
    """
    try:
        wm = bpy.context.window_manager
        if success:
...
class WEBSPIDER_BYOK_OT_fetch_models_catalog(Operator):
    """Refresh the provider+model catalog from the backend"""
    bl_idname = "webspider3d_byok.fetch_models_catalog"
...
classes = (
    WEBSPIDER_BYOK_OT_open_dialog,
    WEBSPIDER_BYOK_OT_save,
    WEBSPIDER_BYOK_OT_codex_load_file,
    WEBSPIDER_BYOK_OT_codex_paste,
    WEBSPIDER_BYOK_OT_request_remove,
    WEBSPIDER_BYOK_OT_cancel_remove,
    WEBSPIDER_BYOK_OT_confirm_remove,
    WEBSPIDER_BYOK_OT_fetch_state,
    WEBSPIDER_BYOK_OT_fetch_models_catalog,
)
