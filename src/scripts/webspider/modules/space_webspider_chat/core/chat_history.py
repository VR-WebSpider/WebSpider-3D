...
"""
Local chat-history archive for WebSpider Chat.

"New Chat" used to hard-clear ``scene.webspider_chat_messages`` with no way
...
(``webspider_chat_history_overlay.cc``, toggled by the header clock button)
lets the user reopen any archived chat.

Storage layout (sidecar — deliberately NOT inside the .webspider3d file, so
project files stay lean, history survives unsaved files, and shared
files never leak conversations):

    ~/.webspider3d/chat_history/<session_id>.json   full archived session
    ~/.webspider3d/chat_history/index.json          metadata index (list cache)
    ~/.webspider3d/chat_media/<session_id>/         copied image files
...
a straight ``restore_propgroup`` into ``scene.webspider_chat_messages``.

Reopening a chat also restores ``scene.webspider_ai_session_id``: the backend
...
def archive_current(scene) -> bool:
    ...
    if DEV_MODE or scene is None:
        return False
    session_id = getattr(scene, "webspider_ai_session_id", "") or ""
    ...
    record = {
        ...
        "user_email": getattr(scene, "webspider_chat_user_id", "") or "",
...
def restore_into_scene(scene, record: dict) -> int:
    ...
    if hasattr(scene, "webspider_chat_layout_epoch"):
        scene.webspider_chat_layout_epoch += 1  # force C++ layout rebuild
