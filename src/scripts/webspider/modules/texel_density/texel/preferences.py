...
# Import removed to avoid circular imports - using string-based class access instead
...
def update_view3d_panel_category(self, context):
	...
	category = self.view3d_panel_category

	if is_panel:
		try:
			# Use string-based class access to avoid circular imports
			panel_class = bpy.types.VIEW3D_PT_texel_density_checker
			bpy.utils.unregister_class(panel_class)
		except Exception as e:
			print(f"[WARNING] Failed to unregister panel {e}")
			pass
		# Update the category and re-register
		panel_class.bl_category = category
...
def update_uv_panel_category(self, context):
	...
	category = self.uv_panel_category

	if is_panel:
		try:
			# Use string-based class access to avoid circular imports
			panel_class = bpy.types.UV_PT_texel_density_checker
			bpy.utils.unregister_class(panel_class)
		except Exception as e:
			print(f"[WARNING] Failed to unregister panel {e}")
			pass
...
def update_save_config(self, context):
	if config_json.saving_enabled:
		config_json.save_addon_prefs()


# get_addon_props function moved to utils.py to avoid circular imports
...
class TDAddonProperties(bpy.types.PropertyGroup):
	offset_x: StringProperty(
		name="",
		description="Offset X from Anchor",
		default="60", update=filter_gradient_offset_x)
...
def register():
	...
	bpy.types.Scene.td_addon_props = bpy.props.PointerProperty(type=TDAddonProperties)
	
	# Initialize with default values and update categories
	# Note: We'll let the first scene access initialize the categories
