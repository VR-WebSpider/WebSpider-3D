...
class WEBSPIDER_AI_OT_imagegen_popup(Operator):
    """Open ImageGen popup dialog"""
    bl_idname = "webspider_ai.imagegen_popup"
    ...
    def invoke(self, context, event):
        ...
        self._original_prompt = context.scene.webspider_ai_imagegen_prompt
        self._original_ref_count = len(context.scene.webspider_ai_imagegen_ref_images)

        return context.window_manager.invoke_popup(self, width=450)

    def check(self, context):
        """Check if generation started - if so, close the popup."""
        if getattr(context.scene, 'webspider_ai_imagegen_is_generating', False):
    ...
    def cancel(self, context):
        ...
        if not self._generation_started:
            if hasattr(self, '_original_prompt'):
                context.scene.webspider_ai_imagegen_prompt = self._original_prompt
    ...
    def _get_selected_image_count(self, context):
        ...
        count = 0
        for item in context.scene.webspider_ai_moodboard_images:
            if item.selected:
                count += 1
        return count
    ...
    def draw(self, context):
        ...
        prompt_row.prop(context.scene, "webspider_ai_imagegen_prompt", text="")
        row.operator("webspider_ai.imagegen_add_style_image", text="", icon='ADD')
        ...
        row.prop(context.scene, "webspider_ai_imagegen_style", text="")
        row.prop(context.scene, "webspider_ai_imagegen_model", text="")
        row.operator("webspider_ai.imagegen_refresh", text="", icon='FILE_REFRESH')

        row = settings_col.row(align=True)
        row.prop(context.scene, "webspider_ai_imagegen_aspect_ratio", text="")
        if hasattr(context.scene, "webspider_ai_imagegen_resolution"):
            row.prop(context.scene, "webspider_ai_imagegen_resolution", text="")

        layout.separator()
        row = layout.row()
        row.scale_y = GENERATE_BUTTON_SCALE_Y
        op = row.operator("webspider_ai.imagegen_generate_and_close", text="Generate", icon='PLAY')
...
class WEBSPIDER_AI_OT_imagegen_generate_and_close(Operator):
    """Generate image and close the popup"""
    bl_idname = "webspider_ai.imagegen_generate_and_close"
    bl_label = "Generate"
    ...
    def execute(self, context):
        prompt = context.scene.webspider_ai_imagegen_prompt.strip()
        if not prompt:
        ...
        context.scene.webspider_ai_imagegen_prompt = ""
...
classes = (
    WEBSPIDER_AI_OT_imagegen_popup,
    WEBSPIDER_AI_OT_imagegen_generate_and_close,
