 ...
 * \brief WebSpider AI3D mode drawing for WebSpider AI space
 */
...
/* -------------------------------------------------------------------- */
/** \name WebSpider AI3D Preview Thumbnail Drawing
 * \{ */
...
/* -------------------------------------------------------------------- */
/** \name WebSpider AI3D Mode Drawing
 * \{ */
...
void webspider_ai_draw_sam3d_mode(const bContext *C, ARegion *region)
{
  ...
  /* Define layout constants using values from webspider_ai_intern.hh */
  ...
  PropertyRNA *image_prop = RNA_struct_find_property(&scene_ptr, "webspider_ai_sam3d_image");
  ...
  if (image && image->source != IMA_SRC_VIEWER) {
    ...
    if (ibuf && (ibuf->float_buffer.data || ibuf->byte_buffer.data)) {
      ...
      PropertyRNA *box_visible_prop = RNA_struct_find_property(&scene_ptr,
                                                               "webspider_ai_sam3d_box_visible");
      if (box_visible_prop && RNA_property_boolean_get(&scene_ptr, box_visible_prop)) {
        PropertyRNA *min_x_prop = RNA_struct_find_property(&scene_ptr, "webspider_ai_sam3d_box_min_x");
        PropertyRNA *min_y_prop = RNA_struct_find_property(&scene_ptr, "webspider_ai_sam3d_box_min_y");
        PropertyRNA *max_x_prop = RNA_struct_find_property(&scene_ptr, "webspider_ai_sam3d_box_max_x");
        PropertyRNA *max_y_prop = RNA_struct_find_property(&scene_ptr, "webspider_ai_sam3d_box_max_y");
    ...
    if (history_prop) {
      ...
      if (history_count > 0) {
        ...
        RNA_PROP_BEGIN (&scene_ptr, itemptr, history_prop) {
          ...
          webspider_ai_draw_sam3d_preview_thumbnail(
