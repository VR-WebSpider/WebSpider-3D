...
"""UI utility functions for WebSpider Chat."""
...
def _sync_bubble_attachment_size(force_attachment_height=False):
    sync_op = getattr(getattr(bpy.ops, "webspider3d", None),
...
def _tag_chat_and_bubble_areas():
    """Tag all WEBSPIDER_AI_CHAT and AGENT_BUBBLE areas for redraw."""
    try:
...
def bump_layout_epoch(scene) -> None:
    ...
    """
    try:
        scene.webspider_chat_layout_epoch = scene.webspider_chat_layout_epoch + 1
    except Exception:
        logger.debug("failed to bump chat layout epoch", exc_info=True)


def redraw_chat_areas():
    """Redraw all WebSpider AI chat surfaces across all windows.
