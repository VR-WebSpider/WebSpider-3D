...
from webspider.modules.moodboard.core.generate_progress import start_progress, reset_progress
...
class WEBSPIDER_AI_OT_scene_recon_pick_image(Operator):
    """Select an image file for scene reconstruction"""

    bl_idname = "webspider_ai.scene_recon_pick_image"
    bl_label = "Pick Image"
...
class WEBSPIDER_AI_OT_scene_recon_remove_image(Operator):
    """Remove the input image from Scene Reconstruction tab"""

    bl_idname = "webspider_ai.scene_recon_remove_image"
...
class WEBSPIDER_AI_OT_scene_recon_generate(Operator):
    """Submit a scene reconstruction job"""

    bl_idname = "webspider_ai.scene_recon_generate"
    ...
    def _start_from_prompt(self, scene, sidebar_tab, prompt):
        ...
        from webspider.modules.moodboard.core.scene_recon_imagegen_bridge import (
    ...
    def _start_from_image_chat(self, scene, image_name, prompt=""):
        ...
        scene.webspider_ai_scene_recon_is_generating = True
        scene.webspider_ai_scene_recon_error = ""
        start_progress('scene_recon')
        ...
        def _on_chat_error(message):
            ...
            def _show():
                try:
                    reset_progress('scene_recon')
                    scene.webspider_ai_scene_recon_is_generating = False
                    scene.webspider_ai_scene_recon_error = message
                    for window in bpy.context.window_manager.windows:
                        for area in window.screen.areas:
                            if area.type in ("WEBSPIDER_AI", "WEBSPIDER_AI_CHAT"):
                                area.tag_redraw()
                except Exception:
                    pass
                return None
...
class WEBSPIDER_AI_OT_scene_recon_cancel(Operator):
    """Cancel the active scene reconstruction job"""

    bl_idname = "webspider_ai.scene_recon_cancel"
    bl_label = "Cancel"
...
class WEBSPIDER_AI_OT_scene_recon_open_folder(Operator):
    """Open the folder containing the result file"""

    bl_idname = "webspider_ai.scene_recon_open_folder"
...
classes = (
    WEBSPIDER_AI_OT_scene_recon_pick_image,
    WEBSPIDER_AI_OT_scene_recon_remove_image,
    WEBSPIDER_AI_OT_scene_recon_generate,
    WEBSPIDER_AI_OT_scene_recon_cancel,
    WEBSPIDER_AI_OT_scene_recon_open_folder,
)
