 ...
 * - webspider_ai_chat_footer_constants.hh: Magic number constants
 * - webspider_ai_chat_footer_cache.cc: Theme and attachment caching
 * - webspider_ai_chat_footer_layout.cc: Height/position calculations
 * - webspider_ai_chat_footer_thumbnails.cc: Thumbnail rendering
 */
 ...
 *
 * Rotation policy (see webspider_ai_chat_footer_next_placeholder):
 ...
 * WebSpider AI chat footer and the floating Agent Bubble footer (both call
 * webspider_ai_chat_footer_region_draw and share scene.webspider_ai_chat_input).
 * \{ */
 ...
 * `is_empty` is whether scene.webspider_ai_chat_input is currently empty. */
static const char *webspider_ai_chat_footer_next_placeholder(bool is_empty)
{
...
void webspider_ai_chat_footer_region_init(wmWindowManager *wm, ARegion *region)
{
  ...
  /* Register the "WebSpider Chat" keymap on the footer region so Cmd+V / Ctrl+V
   * reaches WEBSPIDER_AI_CHAT_OT_paste_image. This is the same keymap the main region
   ...
   * WEBSPIDER_AI_CHAT_OT_paste_image call. */
  wmKeyMap *chat_keymap = WM_keymap_ensure(
      wm->runtime->defaultconf, "WebSpider Chat", SPACE_WEBSPIDER_CHAT, RGN_TYPE_WINDOW);
...
void webspider_ai_chat_footer_region_layout(const bContext *C, ARegion *region)
{
  ...
  int mention_rows = webspider_ai_chat_mention_row_count(scene);
...
void webspider_ai_chat_footer_region_draw(const bContext *C, ARegion *region)
{
  ...
  if (webspider_ai_chat_has_bg_override()) {
    float bg[4];
    webspider_ai_chat_get_bg_override(bg);
  ...
  if (pos.input_w > 0) {
     ...
     * inside ui_but_string_get_ex). Same pattern as webspider_ai_chat_is_busy above. */
    PropertyRNA *input_prop = RNA_struct_find_property(&scene_ptr, "webspider_ai_chat_input");
    if (input_prop) {
      ...
      if (input_but) {
         ...
         * composer returns to empty (see webspider_ai_chat_footer_next_placeholder). */
        const bool input_is_empty = (RNA_property_string_length(&scene_ptr, input_prop) == 0);
        UI_but_placeholder_set(input_but, webspider_ai_chat_footer_next_placeholder(input_is_empty));
  ...
  PropertyRNA *mode_prop = RNA_struct_find_property(&scene_ptr, "webspider_ai_chat_mode");
  int icon_id = ICON_WEBSPIDER_ICON;  /* Default to WebSpider 3D logo */
  ...
  if (mode_prop) {
     ...
     * Note: Using ICON_WEBSPIDER_ICON as fallback until custom SVGs are created */
  ...
  if (is_generate_mode) {
    ...
    PropertyRNA *gen_type_prop = RNA_struct_find_property(&scene_ptr, "webspider_ai_chat_generate_type");
    if (gen_type_prop) {
      /* Get current generate type to determine icon and text */
      int current_gen_value = RNA_property_enum_get(&scene_ptr, gen_type_prop);
      int gen_icon_id = ICON_WEBSPIDER_ICON;  /* Default to WebSpider 3D logo */
      ...
      uiDefIconTextButR(block,
                        ...
                        pos.button_row_height,
                        &scene_ptr,
                        "webspider_ai_chat_generate_type",
                        -1,
                        std::nullopt);
   ...
   * underlying WEBSPIDER_AI_CHAT_OT_attach_moodboard_image operator
  ...
  if (!is_generate_mode && !is_bubble) {
    uiDefIconButO(block,
                  ButType::But,
                  "WEBSPIDER_AI_CHAT_OT_capture_screenshot",
                  blender::wm::OpCallContext::InvokeDefault,
                  ICON_CAMERA_DATA,
                  screenshot_btn_x,
                  pos.buttons_y,
   ...
   * transparent click target running WEBSPIDER_AI_CHAT_OT_toggle_plan_mode, plus
   * a footer_draw_plan_toggle() GPU overlay in the custom-overlay section
   * below. The webspider_ai_chat_plan_enabled property and its toggle operator
  ...
  if (is_generate_mode) {
    ...
    PropertyRNA *gen_type_prop_chk = RNA_struct_find_property(
        &scene_ptr, "webspider_ai_chat_generate_type");
  ...
  if (is_busy) {
    uiDefIconButO(block,
                  ButType::But,
                  "WEBSPIDER_AI_CHAT_OT_abort_session",
                  blender::wm::OpCallContext::InvokeDefault,
                  ICON_CANCEL,
                  pos.send_btn_x,
                  pos.buttons_y,
    ...
    uiDefButO(block,
              ButType::But,
              "WEBSPIDER_AI_CHAT_OT_send_message",
              blender::wm::OpCallContext::InvokeDefault,
              "",
              pos.send_btn_x,
              pos.buttons_y,
  ...
  /* Custom GPU overlays (delegated to webspider_ai_chat_footer_draw.cc) */
