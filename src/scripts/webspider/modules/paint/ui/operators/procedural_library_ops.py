...
"""Procedural material library operators for WebSpider 3D layers system.
...
def _draw_ai_generate_section(layout, wm):
    ...
    prompt_row.prop(wm, "webspider3d_matgen_query", text="",
                    placeholder="e.g. worn copper with green patina")
    prompt_row.operator("matgen.generate_material", text="Generate", icon='PLAY')

    ai_box.row().prop(wm, "webspider3d_matgen_pipeline", text="Pipeline")

    status = wm.webspider3d_matgen_status
    if status:
        row = ai_box.row()
    ...
    recent = wm.webspider3d_matgen_recent
    if not recent:
        return
...
class LAYERS_OT_ProceduralMaterialLibraryPopup(Operator):
    ...
    def draw(self, context):
        ...
        # AI Generate section
        try:
            if hasattr(wm, 'webspider3d_matgen_status'):
