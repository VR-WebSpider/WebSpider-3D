...
"""
WebSpider 3D Properties Space Panel

Panel definition for the WebSpider 3D Properties workspace space.
...
class WEBSPIDER_PROPERTIES_HT_header(Header):
    """Header for the WebSpider 3D Properties space (hosts the space-switch dropdown)."""
    bl_space_type = 'WEBSPIDER_PROPERTIES'
...
class WEBSPIDER_PROPERTIES_PT_main(Panel):
    """Active Layer Properties Panel - Integrated with paint backend"""
    bl_label = ""
    bl_idname = "WEBSPIDER_PROPERTIES_PT_main"
    bl_space_type = 'WEBSPIDER_PROPERTIES'
    ...
    def draw(self, context):
        ...
        editing_idx = webspider3d_ui.editing_mask_index
        if editing_idx >= 0 and editing_idx < len(backend_layer.masks):
            ...
            tab_row.scale_y = 1.5
            tab_row.prop(webspider3d_ui, "mask_panel_tab", expand=True)

            layout.separator(factor=0.5)

            if webspider3d_ui.mask_panel_tab == 'MASK':
            ...
            elif webspider3d_ui.mask_panel_tab == 'BRUSH':
                ...
                draw_brush_tab(context, layout, webspider3d_ui, mp, backend_layer)
        ...
        if backend_layer.type == 'IMAGE':
            ...
            tab_row = layout.row(align=True)
            tab_row.scale_y = 1.5
            tab_row.prop(webspider3d_ui, "main_panel_tab", expand=True)

            layout.separator(factor=0.5)

            if webspider3d_ui.main_panel_tab == 'CHANNELS':
                draw_channels_tab(context, layout, webspider3d_ui, mp, backend_layer)

                # Draw unified transform panel for paint layer (only in CHANNELS tab)
                layout.separator()
                draw_unified_transform_panel(context, layout, backend_layer, mp)
            elif webspider3d_ui.main_panel_tab == 'BRUSH':
                # Brush Texture Generation - visible in BRUSH tab
                _draw_brush_texture_generation(context, layout)
                layout.separator(factor=0.5)
            ...
            _draw_procedural_layer_settings(context, layout, backend_layer, mp, webspider3d_ui)
...
classes = (
    WEBSPIDER_PROPERTIES_HT_header,
