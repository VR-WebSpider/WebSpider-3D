 ...
 * webspider_ai_chat_history_overlay.cc).
 *
...
static int history_hit_row(const WebSpider AIChatRuntime *rt, float mx, float my, bool *r_over_delete)
{
  *r_over_delete = false;
...
static float history_max_scroll(const WebSpider AIChatRuntime *rt)
{
...
static void history_on_search_edit(WebSpider AIChatRuntime *rt)
{
  rt->history_sel = -1;
...
static void history_scroll_selection_into_view(WebSpider AIChatRuntime *rt)
{
...
static void history_open_row(bContext *C, ARegion *region, WebSpider AIChatRuntime *rt, int index)
{
  ...
  rt->history_confirm_id[0] = '\0';
  webspider_ai_chat_history_dispatch_session_op(
      C, region, "webspider_ai_chat.open_history_session", session_id);
  webspider_ai_chat_history_set_visible(C, false);
}

/** Arm-to-confirm delete of a row: first call arms (red "Delete?"),
 * second call on the same row dispatches the delete operator. */
static void history_arm_or_delete_row(bContext *C, ARegion *region, WebSpider AIChatRuntime *rt, int index)
{
  if (index < 0 || index >= int(rt->history_rows.size())) {
    return;
  }
  ...
  if (STREQ(rt->history_confirm_id, session_id)) {
    rt->history_confirm_id[0] = '\0';
    webspider_ai_chat_history_dispatch_session_op(
        C, region, "webspider_ai_chat.delete_history_session", session_id);
  }
  else {
