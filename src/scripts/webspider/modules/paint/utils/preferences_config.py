...
"""JSON-based persistence for WebSpider 3D Paint preferences.
...
def get_prefs_path() -> str:
    """Return the path to the WebSpider 3D Paint preferences JSON file."""
    config_dir = user_resource('CONFIG', path='webspider3d_paint', create=True)
...
def save_preferences() -> None:
    ...
    Reads values from ``bpy.context.scene.webspider3d_paint_preferences`` and
    ...
    prefs = getattr(scene, 'webspider3d_paint_preferences', None)
    if prefs is None:
        return
    ...
    try:
        ...
        logger.debug("WebSpider 3D Paint preferences saved to %s", path)
    except Exception as exc:
        logger.warning("Failed to save WebSpider 3D Paint preferences: %s", exc)
...
def load_preferences() -> None:
    ...
    if not os.path.exists(path):
        logger.debug("No WebSpider 3D Paint preferences file found at %s — using defaults", path)
        return
    ...
    try:
        ...
        logger.warning("Failed to read WebSpider 3D Paint preferences from %s: %s", path, exc)
        return
    ...
    logger.debug("WebSpider 3D Paint preferences loaded from %s", path)
