...
The pattern mirrors `space_webspider_chat/ui/operators/auth_ops.py`.
"""
...
_NETWORK_ERROR_MSG = "Could not reach WebSpider 3D. Check your connection and try again."
...
def fetch_state(
    ...
    threading.Thread(
        target=_thread, daemon=True, name="WebSpider 3DBYOKFetchState"
    ).start()
...
def save_credentials(
    ...
    threading.Thread(
        target=_thread, daemon=True, name="WebSpider 3DBYOKSave"
    ).start()
...
def delete_credentials(
    ...
    threading.Thread(
        target=_thread, daemon=True, name="WebSpider 3DBYOKDelete"
    ).start()
...
def fetch_models_catalog(
    ...
    threading.Thread(
        target=_thread, daemon=True, name="WebSpider 3DBYOKModelsCatalog"
    ).start()
