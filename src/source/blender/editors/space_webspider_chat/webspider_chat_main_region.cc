 ...
 *
 * Main region callbacks for WebSpider Chat space.
...
void webspider_ai_chat_main_region_cursor(wmWindow *win, ScrArea *area, ARegion *region)
{
  ...
  SpaceWebSpider AIChat *swebspider_ai = reinterpret_cast<SpaceWebSpider AIChat *>(area->spacedata.first);
  ...
  if (rt->rules_overlay_active &&
      webspider_ai_chat_rules_cursor(win, rt, region, float(mval[0]), float(mval[1])))
  {
    return;
  }
  ...
  if (rt->history_overlay_active &&
      webspider_ai_chat_history_cursor(win, rt, region, float(mval[0]), float(mval[1])))
  {
    return;
  }
  ...
  blender::Vector<MessageLayoutData> &layout_cache =
      const_cast<blender::Vector<MessageLayoutData> &>(webspider_ai_chat_get_layout_cache(swebspider_ai));
...
static int webspider_ai_chat_ui_handler(bContext *C, const wmEvent *event, void * /*userdata*/)
{
  ...
  if (webspider_ai_chat_rules_handle_event(C, event)) {
   ...
   * closed (runtime flag check, no RNA reads). */
  if (webspider_ai_chat_history_handle_event(C, event)) {
    return WM_UI_HANDLER_BREAK;
  }
  ...
  if (event->type == LEFTMOUSE && event->val == KM_PRESS) {
    ...
    if (webspider_ai_chat_handle_scroll_indicator_click(swebspider_ai, region, mx, my)) {
      ED_region_tag_redraw(region);
      return WM_UI_HANDLER_BREAK;
    }

    /* 2. Empty prompt clicks */
    if (webspider_ai_chat_handle_empty_prompt_click(C, mx, my)) {
      return WM_UI_HANDLER_BREAK;
    }

    /* 3. Action button clicks (copy/retry) */
    if (webspider_ai_chat_handle_action_button_click(C, region, mx, my)) {
      return WM_UI_HANDLER_BREAK;
    }

    /* 4. Steps/thinking collapse toggle clicks */
    if (webspider_ai_chat_handle_steps_click(C, region, mx, my)) {
      return WM_UI_HANDLER_BREAK;
    }

    /* 5. Slot action clicks (approve/modify/cancel/options) */
    if (webspider_ai_chat_handle_slot_action_click(C, region, mx, my)) {
      return WM_UI_HANDLER_BREAK;
    }

    /* 6. Feedback star/comment clicks */
    if (webspider_ai_chat_handle_feedback_click(C, region, mx, my)) {
      return WM_UI_HANDLER_BREAK;
    }
    ...
    /* 7. Option bubble clicks */
    {
      const blender::Vector<MessageLayoutData> &layout_cache =
          webspider_ai_chat_get_layout_cache(swebspider_ai);
      ...
      for (const MessageLayoutData &layout : layout_cache) {
        for (int i = 0; i < layout.option_bubble_count; i++) {
              ...
              BLI_rctf_isect_pt(&bubble.bounds, view_x, view_y))
          {
            ...
            wmOperatorType *ot = WM_operatortype_find("webspider_ai_chat.select_slot_action", true);
...
static void webspider_ai_chat_ui_handler_remove(bContext * /*C*/, void * /*userdata*/)
{
  /* Nothing to free */
}
...
void webspider_ai_chat_main_region_init(wmWindowManager *wm, ARegion *region)
{
   ...
   * webspider_ai_chat_ui_handler gets first look at every mouse press. That works
  ...
  WM_event_remove_ui_handler(
      &region->runtime->handlers, webspider_ai_chat_ui_handler, webspider_ai_chat_ui_handler_remove, nullptr, false);
  WM_event_add_ui_handler(nullptr,
                          &region->runtime->handlers,
                          webspider_ai_chat_ui_handler,
                          webspider_ai_chat_ui_handler_remove,
                          nullptr,
                          eWM_EventHandlerFlag(0));

  /* Register the View2D keymap BEFORE the WebSpider Chat keymap. Keymap
   * handlers run in registration order, and both keymaps bind LEFTMOUSE:
   * "WebSpider Chat" for text selection and "View2D" for scrollbar dragging
  ...
  /* Register WebSpider Chat keymap for text selection (modal drag) and copy. */
  wmKeyMap *webspider_ai_keymap = WM_keymap_ensure(
      wm->runtime->defaultconf, "WebSpider Chat", SPACE_WEBSPIDER_CHAT, RGN_TYPE_WINDOW);
  WM_event_add_keymap_handler(&region->runtime->handlers, webspider_ai_keymap);
...
static void webspider_ai_chat_clear_background()
{
...
{
  webspider_ai_chat_clear_background();
  webspider_ai_chat_draw_messages(C, region);
  /* Past-chats overlay — drawn last (screen-space) so it sits on top of
   * messages, the empty state, and the View2D scrollbar. */
  webspider_ai_chat_draw_history_overlay(C, region);
   ...
   * braces for the event-order contract in webspider_ai_chat_ui_handler). */
  webspider_ai_chat_draw_rules_overlay(C, region);
}
 ...
 * NC_SPACE | ND_SPACE_WEBSPIDER_CHAT_TICK at CHAT_ANIM_PUMP_FPS; the (narrow)
 * main-region listener turns each tick into a redraw for every chat surface
 * (WEBSPIDER_AI_CHAT editor and the floating agent bubble).
 *
 * Lifecycle: every messages draw calls webspider_ai_chat_anim_pump_request() with
    ...
    if (g_chat_anim_timer == nullptr) {
      ...
      if (win != nullptr) {
        g_chat_anim_timer = WM_event_timer_add_notifier(
            wm, win, NC_SPACE | ND_SPACE_WEBSPIDER_CHAT_TICK, 1.0 / CHAT_ANIM_PUMP_FPS);
      }
    }
  }
...
void webspider_ai_chat_main_region_exit(wmWindowManager *wm, ARegion * /*region*/)
{
  ...
  webspider_ai_chat_anim_pump_shutdown(wm);
}
...
void webspider_ai_chat_main_region_listener(const wmRegionListenerParams *params)
{
  ...
  switch (wmn->category) {
    case NC_SPACE:
      ...
      if (wmn->data == ND_SPACE_WEBSPIDER_CHAT || wmn->data == ND_SPACE_AGENT_BUBBLE ||
          wmn->data == ND_SPACE_WEBSPIDER_CHAT_TICK)
      {
        /* WEBSPIDER_AI_CHAT_TICK is the animation frame pump (see above): it only
