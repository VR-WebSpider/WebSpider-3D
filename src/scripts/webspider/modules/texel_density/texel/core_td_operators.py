...
def find_image_editor_area(context):
	"""Find IMAGE_EDITOR area, preferably in WEBSPIDER_UV mode"""
	# First try to find IMAGE_EDITOR in WEBSPIDER_UV mode
	for area in context.screen.areas:
		if area.type == 'IMAGE_EDITOR':
			sima = area.spaces.active
			if sima and hasattr(sima, 'mode') and sima.mode == 'WEBSPIDER_UV':
				return area
	# Fallback to any IMAGE_EDITOR
	for area in context.screen.areas:
		if area.type == 'IMAGE_EDITOR':
			return area
	return None
...
def is_image_editor_context(context):
	"""Check if current context has IMAGE_EDITOR area active"""
	if context.area and context.area.type == 'IMAGE_EDITOR':
		return True
	# Also check if we can find an IMAGE_EDITOR area
...
class TexelDensityCheck(bpy.types.Operator):
	...
	def execute(self, context):
		...
		# Find IMAGE_EDITOR area for context checks
		img_area = find_image_editor_area(context)
		has_uv_select_sync = context.scene.tool_settings.use_uv_select_sync
		...
		for obj in start_selected_obj:
			...
			elif img_area and not has_uv_select_sync:
				bm.clear()
...
class TexelDensitySet(bpy.types.Operator):
	...
	def execute(self, context):
		...
		# Find IMAGE_EDITOR area for operations
		img_area = find_image_editor_area(context)
		if not img_area:
			self.report({'ERROR'}, "No IMAGE_EDITOR area found")
			return {'CANCELLED'}
		...
		for obj in start_selected_obj:
			...
			# UV Editor setup - get cursor location before context override
			...
			# Get the window region and space_data for the IMAGE_EDITOR area (needed for proper context)
			img_region = None
			for region in img_area.regions:
				if region.type == 'WINDOW':
					img_region = region
					break
			
			img_space_data = img_area.spaces.active

			# Use context override for IMAGE_EDITOR operations (like UV editor operators)
			# Include space_data in override to ensure proper context (like bake operators do)
			override_dict = {'area': img_area, 'space_data': img_space_data}
			if img_region:
				override_dict['region'] = img_region
			
			with context.temp_override(**override_dict):
				# Check if we're in IMAGE_EDITOR and sync selection if needed
				if not start_sync_selection:
				...
				# Access space_data through context (inside override)
				sima = context.space_data
				if sima.image and sima.image.name == 'Render Result':
					sima.image = None

				# Preserve original mode (WEBSPIDER_UV or UV) - don't change it
				start_space_mode = sima.mode
				# Only set to UV if it's not already in a UV editing mode
				if sima.mode not in ('UV', 'WEBSPIDER_UV'):
					sima.mode = 'UV'

				start_cursor_loc = sima.cursor_location.copy()
				start_pivot_mode = sima.pivot_point

				# Set pivot anchor if needed
				if td.rescale_anchor != 'SELECTION':
					sima.pivot_point = 'CURSOR'
				...
				sima.pivot_point = start_pivot_mode
				
				# Restore original space mode (preserve WEBSPIDER_UV if it was that)
				sima.mode = start_space_mode
