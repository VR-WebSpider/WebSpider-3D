...
Operators:
- WEBSPIDER_AI_OT_hunyuan_load_image:      File browser to load images
- WEBSPIDER_AI_OT_hunyuan_remove_image:    Clear the main image reference
- WEBSPIDER_AI_OT_hunyuan_add_multi_view:  Add a multi-view image slot (Pro)
- WEBSPIDER_AI_OT_hunyuan_remove_multi_view: Remove a multi-view slot by index
- WEBSPIDER_AI_OT_hunyuan_generate:        Main generate operator (per-mode)
- WEBSPIDER_AI_OT_hunyuan_cancel:          Cancel running job (stops poll timer)
"""
...
from webspider.modules.common.job_queue.core.model_io import redraw_3d_views
...
class WEBSPIDER_AI_OT_hunyuan_load_image(Operator):
    """Load an image file for Hunyuan"""

    bl_idname = "webspider_ai.hunyuan_load_image"
    bl_label = "Load Image"
...
class WEBSPIDER_AI_OT_hunyuan_remove_image(Operator):
    """Clear the main image reference for Hunyuan"""

    bl_idname = "webspider_ai.hunyuan_remove_image"
...
class WEBSPIDER_AI_OT_hunyuan_remove_uploaded_image(Operator):
    """Remove an uploaded image from Pro mode by index"""

    bl_idname = "webspider_ai.hunyuan_remove_uploaded_image"
...
class WEBSPIDER_AI_OT_hunyuan_add_multi_view(Operator):
    """Add a multi-view image slot"""

    bl_idname = "webspider_ai.hunyuan_add_multi_view"
    bl_label = "Add View"
    bl_options = {'REGISTER', 'UNDO'}
...
class WEBSPIDER_AI_OT_hunyuan_remove_multi_view(Operator):
    """Remove a multi-view image slot"""

    bl_idname = "webspider_ai.hunyuan_remove_multi_view"
...
class WEBSPIDER_AI_OT_hunyuan_generate(Operator):
    """Generate 3D using Hunyuan"""

    bl_idname = "webspider_ai.hunyuan_generate"
    bl_label = "Generate"
    bl_options = {'REGISTER'}
    ...
    def execute(self, context):
        ...
        if mode == 'PRO':
            try:
                ...
                return {'CANCELLED'}
            from webspider.modules.common.job_queue.constants import FEATURE_IMAGE_TO_3D_PRO
        ...
        if mode == 'RAPID':
            try:
                ...
                self.report({'ERROR'}, str(e))
                return {'CANCELLED'}
            from webspider.modules.common.job_queue.constants import FEATURE_HUNYUAN_RAPID
            from webspider.modules.common.job_queue.ui.lists.queue_uilist import mark_enqueued
    ...
    def _submit_pro_direct(self, context):
        ...
        """
        from webspider.modules.moodboard.core.generation_enqueue import enqueue_pro_job
...
class WEBSPIDER_AI_OT_hunyuan_cancel(Operator):
    """Cancel the running Hunyuan job"""

    bl_idname = "webspider_ai.hunyuan_cancel"
    bl_label = "Cancel"
    bl_options = {'REGISTER'}
...
class WEBSPIDER_AI_OT_hunyuan_dismiss_error(Operator):
    """Dismiss the error and return to idle"""

    bl_idname = "webspider_ai.hunyuan_dismiss_error"
    bl_label = "Dismiss"
    bl_options = {'REGISTER'}
...
classes = (
    WEBSPIDER_AI_OT_hunyuan_load_image,
    WEBSPIDER_AI_OT_hunyuan_remove_image,
    WEBSPIDER_AI_OT_hunyuan_remove_uploaded_image,
    WEBSPIDER_AI_OT_hunyuan_add_multi_view,
    WEBSPIDER_AI_OT_hunyuan_remove_multi_view,
    WEBSPIDER_AI_OT_hunyuan_generate,
    WEBSPIDER_AI_OT_hunyuan_cancel,
    WEBSPIDER_AI_OT_hunyuan_dismiss_error,
)
