...
- add_image_to_moodboard() stamping webspider3d_created_at_iso
- _get_moodboard_image_info() exposing webspider3d_created_at_iso
"""
...
def _make_moodboard_item(
    ...
    *,
    ...
    """Build a mock WebSpiderAIMoodboardImage PropertyGroup item."""
    item = MagicMock()
...
class TestListMoodboardImages(unittest.TestCase):
    ...
    def test_each_entry_includes_webspider3d_created_at_iso(self):
        from webspider.modules.moodboard.core.moodboard_utils import list_moodboard_images
        items = [_make_moodboard_item("a.png", created_at_iso="2026-05-08T10:00:00+00:00")]
        ctx = _make_context(items)
        result = list_moodboard_images(ctx)
        self.assertEqual(result["images"][0]["webspider3d_created_at_iso"], "2026-05-08T10:00:00+00:00")
    ...
    def test_no_moodboard_collection_returns_empty(self):
        from webspider.modules.moodboard.core.moodboard_utils import list_moodboard_images
        scene = MagicMock(spec=[])  # no webspider_ai_moodboard_images attr
        ctx = MagicMock()
        ctx.scene = scene
        result = list_moodboard_images(ctx)
...
class TestGetMoodboardImageInfoIncludesTimestamp(unittest.TestCase):
    """The shared formatter must surface webspider3d_created_at_iso."""
...
class TestAddImageToMoodboardStampsTimestamp(unittest.TestCase):
    """add_image_to_moodboard must stamp webspider3d_created_at_iso on insertion."""

    def test_stamps_iso_timestamp(self):
        from webspider.modules.common.utils import image_utils

        added_item = MagicMock()
        added_item.webspider3d_created_at_iso = ""
        scene = MagicMock()
        scene.webspider_ai_moodboard_images = MagicMock()
        scene.webspider_ai_moodboard_images.add.return_value = added_item
        ...
        self.assertNotEqual(added_item.webspider3d_created_at_iso, "")
        # Loose ISO-8601 sniff: "YYYY-MM-DD" prefix
        self.assertRegex(added_item.webspider3d_created_at_iso, r"^\d{4}-\d{2}-\d{2}T")
