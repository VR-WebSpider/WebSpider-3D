 ...
 * draw (webspider_ai_chat_history_overlay.cc) and event
 * (webspider_ai_chat_history_events.cc) halves: RNA readers for the
...
{
  PointerRNA wm_ptr = RNA_id_pointer_create(&wm->id);
  PropertyRNA *entries_prop = RNA_struct_find_property(&wm_ptr, "webspider_ai_chat_history_entries");
  if (!entries_prop) {
    return;
  }
