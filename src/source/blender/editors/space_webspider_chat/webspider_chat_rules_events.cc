 ...
 * webspider_ai_chat_rules_overlay.cc).
 *
 ...
 *   - Enter or the Submit button saves: webspider_ai_chat.rule_add for a new
 *     rule, webspider_ai_chat.rule_update while editing an existing card;
 ...
 *     rule_set_scope (moving the rule between the ~/.webspider3d disk store
...
static float rules_list_max_scroll(const WebSpider AIChatRuntime *rt)
{
...
static bool rules_sel_range(const WebSpider AIChatRuntime *rt, int *r_from, int *r_to)
{
...
static void rules_drop_selection(ARegion *region, WebSpider AIChatRuntime *rt)
{
  if (rt->rules_sel_anchor >= 0) {
...
static void rules_sel_extend_begin(WebSpider AIChatRuntime *rt)
{
  if (rt->rules_sel_anchor < 0) {
...
static void rules_remove_raw(WebSpider AIChatRuntime *rt, int from, int to)
{
...
static int rules_offset_from_mouse(const WebSpider AIChatRuntime *rt, float mx, float my)
{
  ...
  return webspider_ai_chat_rules_x_to_offset(rt, line, mx - text_x);
}

/** Reset the composer to empty new-rule mode. */
static void rules_reset_composer(WebSpider AIChatRuntime *rt)
{
  ...
  webspider_ai_chat_rules_relayout(rt);
}

/** Relayout + caret-follow after any local text change (data is only
 * persisted on submit — see rules_submit). */
static void rules_after_edit(ARegion *region, WebSpider AIChatRuntime *rt)
{
  webspider_ai_chat_rules_relayout(rt);
  rt->rules_caret_goal_x = -1.0f;
  webspider_ai_chat_rules_editor_follow_caret(rt, rt->rules_editor_view_h);
  ED_region_tag_redraw(region);
}

/** Insert `len` bytes at the caret, capacity permitting. */
static void rules_insert(ARegion *region, WebSpider AIChatRuntime *rt, const char *str, int len)
{
...
static void rules_delete_range(ARegion *region, WebSpider AIChatRuntime *rt, int from, int to)
{
...
static void rules_paste(ARegion *region, WebSpider AIChatRuntime *rt)
{
  int buf_len = 0;
...
static void rules_caret_vertical(ARegion *region, WebSpider AIChatRuntime *rt, bool down)
{
  const int line_count = int(rt->rules_lines.size());
  if (line_count == 0) {
    return;
  }
  const int line = webspider_ai_chat_rules_line_of_offset(rt, rt->rules_cursor);
  if (rt->rules_caret_goal_x < 0.0f) {
    rt->rules_caret_goal_x = webspider_ai_chat_rules_offset_to_x(rt, line, rt->rules_cursor);
  }
  const int target = std::clamp(line + (down ? 1 : -1), 0, line_count - 1);
  if (target != line) {
    rt->rules_cursor = webspider_ai_chat_rules_x_to_offset(rt, target, rt->rules_caret_goal_x);
    webspider_ai_chat_rules_editor_follow_caret(rt, rt->rules_editor_view_h);
    ED_region_tag_redraw(region);
  }
}
...
static void rules_submit(bContext *C, ARegion *region, WebSpider AIChatRuntime *rt)
{
  if (!rules_buffer_has_content(rt)) {
    return;
  }
  if (rt->rules_editing_index >= 0) {
    webspider_ai_chat_rules_dispatch_op(
        C, region, "webspider_ai_chat.rule_update", rt->rules_editing_index, rt->rules_text);
  }
  else {
    webspider_ai_chat_rules_dispatch_op(C, region, "webspider_ai_chat.rule_add", -1, rt->rules_text);
  }
...
static void rules_begin_edit(bContext *C, WebSpider AIChatRuntime *rt, int index)
{
  blender::Vector<RuleDrawEntry> entries;
  webspider_ai_chat_rules_read_entries(CTX_wm_manager(C), entries);
...
static void rules_arm_or_delete(bContext *C, ARegion *region, WebSpider AIChatRuntime *rt, int index)
{
  if (rt->rules_confirm_delete == index) {
    ...
    webspider_ai_chat_rules_dispatch_op(C, region, "webspider_ai_chat.rule_delete", index, nullptr);
  }
  else {
  ...
  switch (event->type) {
    case EVT_ESCKEY:
      ...
      if (rt->rules_editing_index >= 0) {
        ...
        webspider_ai_chat_rules_set_visible(C, false);
      }
      return true;
...
bool webspider_ai_chat_rules_handle_event(bContext *C, const wmEvent *event)
{
  ...
  SpaceWebSpider AIChat *swebspider_ai = static_cast<SpaceWebSpider AIChat *>(area->spacedata.first);
    ...
    if (BLI_rctf_isect_pt(&rt->rules_list_bounds, mx, my)) {
      for (const RuleRowHit &row : rt->rules_rows) {
        ...
        if (BLI_rctf_isect_pt(&row.toggle_bounds, mx, my)) {
          rt->rules_confirm_delete = -1;
          webspider_ai_chat_rules_dispatch_op(
              C, region, "webspider_ai_chat.rule_toggle", row.index, nullptr);
        }
          ...
          webspider_ai_chat_rules_dispatch_op(
              C, region, "webspider_ai_chat.rule_set_scope", row.index, nullptr);
        }
