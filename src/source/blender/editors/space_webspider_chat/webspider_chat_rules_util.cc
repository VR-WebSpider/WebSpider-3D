 ...
 * Python-owned properties (WindowManager.webspider_ai_chat_rules_visible + the
 * webspider_ai_chat_rule_entries mirror), the operator dispatch used for every
 * data mutation, greedy word-wrap line layout, and caret geometry used by
 * both the draw (webspider_ai_chat_rules_overlay.cc) and event
 * (webspider_ai_chat_rules_events.cc) halves.
 */
...
bool webspider_ai_chat_rules_read_visible(wmWindowManager *wm)
{
  ...
  PropertyRNA *prop = RNA_struct_find_property(&wm_ptr, "webspider_ai_chat_rules_visible");
  if (!prop) {
    return false;
  }
...
{
  PointerRNA wm_ptr = RNA_id_pointer_create(&wm->id);
  PropertyRNA *entries_prop = RNA_struct_find_property(&wm_ptr, "webspider_ai_chat_rule_entries");
  if (!entries_prop) {
    return;
  }
...
float webspider_ai_chat_rules_relayout(WebSpider AIChatRuntime *rt)
{
  ...
  const int count = webspider_ai_chat_rules_wrap_text(
