...
from webspider.modules.common.utils.webspider_ai_space_utils import count_selected_moodboard_images
...
from webspider.modules.moodboard.constants import SEP_INTRA, SEP_SECTION
...
def _draw_imagegen(layout, context):
    ...
    col = draw_section_box(
        ...
        "Reference Images",
        icon='IMAGE_DATA',
        action_op="webspider_ai.imagegen_upload_reference",
    )
    ...
    for ref_item in tab.reference_images:
        if ref_item.image:
            draw_image_info_card(
                col, ref_item.image,
                remove_op="webspider_ai.imagegen_remove_reference_image",
    ...
    row.operator("webspider_ai.imagegen_refresh", text="", icon='FILE_REFRESH')
    ...
    try:
        from webspider.modules.common.generation_params import draw_service_params
    ...
    # --- Generate ---
    draw_generate_footer(layout, context, "webspider_ai.imagegen_generate", "imagegen",
...
def _draw_lookdev360(layout, context):
    ...
    tab = context.scene.webspider_ai_moodboard_sidebar.tab_lookdev360
    ...
    # --- Generate ---
    draw_generate_footer(layout, context, "webspider_ai.lookdev360_generate", "lookdev360",
...
def _draw_image_to_3d_basic(layout, context):
    ...
    # --- Generate ---
    draw_generate_footer(layout, context, "webspider_ai.image_to_3d_generate", "image_to_3d",
...
def _draw_scene_recon(layout, context):
    ...
    col = draw_section_box(
        ...
        "Input Image",
        icon='IMAGE_DATA',
        action_op="webspider_ai.scene_recon_pick_image",
    )
    draw_moodboard_image_toggle(col, tab, context)
    ...
    if not tab.use_selected_image:
        if tab.image_name:
            ...
            draw_image_info_card(
                col, img,
                remove_op="webspider_ai.scene_recon_remove_image",
    ...
    # Progress (if running)
    if scene.webspider_ai_scene_recon_is_generating:
    ...
    # --- Generate ---
    draw_generate_footer(layout, context, "webspider_ai.scene_recon_generate", "scene_recon",
                         cancel_op="webspider_ai.scene_recon_cancel",
...
def _draw_scene_gen_exp(layout, context):
    ...
    scene = context.scene
    tab = scene.webspider_ai_moodboard_sidebar.tab_scene_gen_exp
    is_processing = getattr(scene, 'webspider_ai_scene_gen_exp_is_processing', False)
    ...
    row.operator("webspider_ai.scene_gen_exp_pick_image", text="", icon='FILE_FOLDER')
    col.separator(factor=SEP_INTRA)
    ...
    if not tab.use_selected_image and tab.reference_image:
        draw_image_info_card(
            col, tab.reference_image,
            remove_op="webspider_ai.scene_gen_exp_remove_image",
        )
    ...
    if is_processing:
        ...
        btn_row.operator(
            "webspider_ai.scene_gen_exp_extract_labels",
    ...
    if tab.has_result and not is_processing:
        ...
        row.operator("webspider_ai.scene_gen_exp_clear", text="", icon='X')
    ...
    if len(tab.objects) > 0:
        ...
        header.operator("webspider_ai.scene_gen_exp_toggle_all", text="", icon='CHECKBOX_HLT')
        list_box.template_list(
            "WEBSPIDER_AI_UL_scene_gen_labels", "",
            tab, "objects",
    ...
    if gen_in_progress:
        ...
        btn_row.operator(
            "webspider_ai.scene_gen_exp_generate_images",
...
def _draw_queue(layout, context):
    ...
    if done_terminal:
        layout.separator(factor=SEP_INTRA)
        layout.operator(
            "webspider_ai.queue_clear_all_completed",
