...
1. Registers the bubble menu type (WEBSPIDER_AI_CHAT_MT_agent_bubble) SYNCHRONOUSLY
...
SpaceWebSpider AIChat editor's spacetype init (webspider_chat_operatortypes in
space_webspider_chat.cc) — that path runs during Blender startup, before any
Python addon. So by the time anything in this file runs, bpy.ops.webspider_ai_chat
...
from webspider.modules.agent_bubble.core import pill_icons
from webspider.modules.agent_bubble.core.bubble_autoshow import (
...
from webspider.modules.agent_bubble.core.bubble_splash_watch import (
    SPLASH_POLL_S as _SPLASH_POLL_S,
    splash_watch_tick as _splash_watch_tick,
)
from webspider.modules.agent_bubble.core import bubble_state as _st
from webspider.modules.agent_bubble.ui.menus.agent_bubble_menu import (
    classes as bubble_menu_classes,
)
from webspider.modules.agent_bubble.ui.operators.bubble_close_op import (
    classes as bubble_close_op_classes,
)
from webspider.modules.agent_bubble.ui.operators.bubble_header_drag_op import (
    classes as bubble_header_drag_classes,
)
from webspider.modules.agent_bubble.ui.operators.bubble_history_ops import (
    classes as bubble_op_classes,
)
from webspider.modules.agent_bubble.ui.operators.window_ops import (
    classes as window_op_classes,
)
from webspider.modules.agent_bubble.ui.header import (
    classes as header_classes,
)
from webspider.modules.agent_bubble.ui.panels.footer_panel import (
    classes as footer_panel_classes,
)
from webspider.modules.agent_bubble.ui.panels.history_panel import (
...
def mark_user_closed() -> None:
    """Public API used by WEBSPIDER_OT_bubble_close."""
...
def _streaming_redraw_tick():
    ...
    try:
        ...
        state = (
            getattr(scene, "webspider_chat_state", "OFFLINE")
...
def _register_bubble_keymap() -> None:
    ...
    try:
        ...
        for value in ('PRESS', 'CLICK'):
            kmi = km.keymap_items.new(
                idname="webspider.bubble_block_context_menu",
                type='RIGHTMOUSE',
                value=value,
            )
            _keymap_items.append((km, kmi))
    ...
    try:
        km = kc.keymaps.new(name="Window", space_type='EMPTY')
        kmi = km.keymap_items.new(
            idname="webspider.bubble_header_drag",
            type='LEFTMOUSE',
            value='PRESS',
        )
        _keymap_items.append((km, kmi))
    ...
    try:
        ...
        km = kc.keymaps.new(name="Window", space_type='EMPTY')
        kmi = km.keymap_items.new(
            idname="webspider.bubble_toggle_minimise",
            type='B',
            value='PRESS',
            shift=True,
            oskey=is_macos,
...
def _draw_topbar_open_agent(self, context):
    ...
    layout = self.layout
    layout.separator()
    # SPARKLE is a WebSpider 3D color SVG icon (UI_icons.hh WEBSPIDER_AI CHAT block);
    # fall back to the old bulb on builds that predate it.
    try:
        layout.operator(
            "webspider.agent_bubble_open_window",
            text="Open WebSpider AI",
            icon='SPARKLE',
        )
    except TypeError:
        layout.operator(
    ...
    # Update badge — appears just right of Open WebSpider AI whenever an update
    # is known; clicking re-shows the sticky update toast.
    try:
        from webspider.modules.common.updates.ui.topbar_badge import (
            draw_update_badge,
        )
...
def register():
    ...
    if not hasattr(bpy.types.Scene, "webspider3d_bubble_history_lines"):
        bpy.types.Scene.webspider3d_bubble_history_lines = bpy.props.IntProperty(
    ...
    if not hasattr(bpy.types.Scene, "webspider3d_bubble_history_active_index"):
        bpy.types.Scene.webspider3d_bubble_history_active_index = bpy.props.IntProperty(
...
def unregister():
    ...
    if hasattr(bpy.types.Scene, "webspider3d_bubble_history_lines"):
        try:
            del bpy.types.Scene.webspider3d_bubble_history_lines
        except Exception as e:  # noqa: BLE001
            logger.warning("agent_bubble: removing history flag failed: %s", e)

    if hasattr(bpy.types.Scene, "webspider3d_bubble_history_active_index"):
        try:
            del bpy.types.Scene.webspider3d_bubble_history_active_index
        except Exception as e:  # noqa: BLE001
