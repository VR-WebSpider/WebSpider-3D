...
"""
Chat history operators for WebSpider Chat — Python side of the past-chats
overlay.

"New Chat" archives the current conversation to ~/.webspider3d/chat_history/
(see ``core/chat_history.py``). The list UI itself is a custom
C++-drawn overlay in the chat main region
(``editors/space_webspider_chat/webspider_chat_history_overlay.cc``) — WebSpider 3D
styling, hover states, its own scrolling. This module owns the data
side of the contract:

  * ``WindowManager.webspider_chat_history_visible`` — overlay visibility.
    Toggled by ``WEBSPIDER_AI_CHAT_OT_show_history`` (header clock button);
    the C++ side also clears it on ESC / click-away / row open.
  * ``WindowManager.webspider_chat_history_entries`` — runtime mirror of the
  ...
  * ``webspider_ai_chat.open_history_session`` / ``webspider_ai_chat.delete_history_session``
    — dispatched by the C++ overlay with a ``session_id`` string prop
    (same pattern as slot-action clicks).

Reopening restores the exact transcript into ``scene.webspider_chat_messages``
and sets ``scene.webspider_ai_session_id`` back, so the next message resumes the
...
class WebSpiderAIChatHistoryEntry(PropertyGroup):
...
def sync_history_entries(context) -> None:
    ...
    scene = context.scene
    user_email = getattr(scene, "webspider_chat_user_id", "") if scene else ""

    entries = wm.webspider_chat_history_entries
    entries.clear()
    try:
...
class WEBSPIDER_AI_CHAT_OT_show_history(Operator):
    """Toggle the past-chats overlay (syncs the list when opening)."""

    bl_idname = "webspider_ai_chat.show_history"
    ...
    def execute(self, context):
        wm = context.window_manager
        opening = not wm.webspider_chat_history_visible
        if opening:
            ...
            if getattr(wm, 'webspider_chat_rules_visible', False):
                wm.webspider_chat_rules_visible = False
        wm.webspider_chat_history_visible = opening
        redraw_chat_areas()
        return {'FINISHED'}


class WEBSPIDER_AI_CHAT_OT_open_history_session(Operator):
    """Reopen an archived chat session (dispatched by the C++ overlay)."""

    bl_idname = "webspider_ai_chat.open_history_session"
    bl_label = "Open Chat"
    bl_description = (
    ...
    def execute(self, context):
        ...
        if not self.session_id:
            return {'CANCELLED'}
        if getattr(scene, "webspider_ai_session_id", "") == self.session_id:
        ...
        # WEBSPIDER_AI_CHAT_OT_new_session.
        ...
        wm = context.window_manager
        if hasattr(wm, "webspider_chat_history_visible"):
...
class WEBSPIDER_AI_CHAT_OT_delete_history_session(Operator):
    """Delete an archived chat session from history."""

    bl_idname = "webspider_ai_chat.delete_history_session"
...
classes = (
    WebSpiderAIChatHistoryEntry,
    WEBSPIDER_AI_CHAT_OT_show_history,
    WEBSPIDER_AI_CHAT_OT_open_history_session,
    WEBSPIDER_AI_CHAT_OT_delete_history_session,
)
...
def register():
    ...
    # Runtime mirror of ~/.webspider3d/chat_history read by the C++ overlay.
    # Session state, never persisted.
    bpy.types.WindowManager.webspider_chat_history_entries = CollectionProperty(
        type=WebSpiderAIChatHistoryEntry,
    ...
    bpy.types.WindowManager.webspider_chat_history_visible = BoolProperty(
...
def unregister():
    for attr in ("webspider_chat_history_entries", "webspider_chat_history_visible"):
        try:
