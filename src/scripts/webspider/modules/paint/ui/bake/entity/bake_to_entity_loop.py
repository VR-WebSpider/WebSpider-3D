...
"""Bake loop helper functions for entity baking."""
...
from ..utils.bake_common import TEMP_EMISSION
...
def setup_other_object_channel_bake(
    ...
    Parameters:
        ...
        idx: Channel index
        other_objs: List of other objects
        ch_other_objects: Channel other objects data
        ch_other_mats: Channel other materials data
        ch_other_defaults: Channel defaults data
        ch_other_default_weights: Channel default weights data
        ch_other_sockets: Channel sockets data
        all_other_mats: All other materials
...
def setup_other_object_emission_bake(other_mats, other_defaults, other_sockets):
    ...
    Parameters:
        other_mats: List of other materials
        other_defaults: Default values
        other_sockets: Socket connections
    """
...
def set_overwrite_filepath(image, bprops, mp, idx, ch_ids, overwrite_image_name, segment):
    ...
    Parameters:
        image: Image to set filepath for
        bprops: Bake properties
        mp: Material properties
        idx: Channel index
        ch_ids: List of channel IDs
        overwrite_image_name: Name of image being overwritten
...
def handle_main_image_result(
    ...
    Parameters:
        ...
        image: Baked image
        segment: Image atlas segment
        do_overwrite: Whether overwriting existing image
        overwrite_image_name: Name of image being overwritten
        channel_idx: Channel index
        new_segment_created: Whether a new segment was created
    ...
    if bprops.target_type == "PREVIEW":
        # Mark image as baked so it appears in the preview section
        bi = image.m_bake_info
        if not bi.is_baked:
            bi.is_baked = True
        if bi.bake_type != bprops.type:
            bi.bake_type = bprops.type
        # Set fake user to prevent image from being deleted when not in use
...
def log_bake_result(image, time_elapsed):
    """Log bake result.

    Parameters:
        image: Baked image or None
        time_elapsed: Time taken to bake
    """
    if image:
        logger.info(
