...
"""
Clipboard operators for WebSpider Chat.

Handles paste operations for text and images.

Note: The per-bubble copy button is handled entirely in C++
(webspider_chat_hit_testing.cc → WM_clipboard_text_set). These Python
...
class WEBSPIDER_AI_CHAT_OT_paste_text(Operator):
    """Paste text from clipboard into chat input field"""
    bl_idname = "webspider_ai_chat.paste_text"
    bl_label = "Paste Text"
    bl_options = {'REGISTER'}
    ...
    def execute(self, context):
        ...
        # Get current input
        current_input = scene.webspider_chat_input
        ...
        scene.webspider_chat_input = new_input
        redraw_chat_areas()
...
class WEBSPIDER_AI_CHAT_OT_paste_image(Operator):
    """Paste image from clipboard as attachment"""
    bl_idname = "webspider_ai_chat.paste_image"
    ...
    def _paste_image_from_clipboard(self):
        ...
        temp_path = os.path.join(
            screenshots_dir, f"webspider_ai_paste_{uuid.uuid4().hex}.png"
        )
...
classes = (
    WEBSPIDER_AI_CHAT_OT_paste_text,
