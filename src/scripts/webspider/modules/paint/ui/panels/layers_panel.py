...
"""
WebSpider 3D Layers Space Panel

Panel definition for the WebSpider 3D Layers workspace space.
...
class WEBSPIDER_LAYERS_HT_header(Header):
    """Header containing toolbar buttons"""
    bl_idname = "WEBSPIDER_LAYERS_HT_header"
    bl_space_type = 'WEBSPIDER_LAYERS'
...
class WEBSPIDER_LAYERS_HT_footer(Header):
    """Footer containing move up/down buttons"""
    bl_idname = "WEBSPIDER_LAYERS_HT_footer"
    bl_space_type = 'WEBSPIDER_LAYERS'
    ...
    def draw(self, context):
        ...
        is_editing_mask = (
            hasattr(webspider3d_ui, 'editing_mask_index')
            and webspider3d_ui.editing_mask_index >= 0
        )
...
class WEBSPIDER_LAYERS_PT_main(Panel):
    """Texture Layer Stack Panel - Integrated with paint backend"""
    bl_label = ""
    bl_idname = "WEBSPIDER_LAYERS_PT_main"
    bl_space_type = 'WEBSPIDER_LAYERS'
    ...
    def draw(self, context):
        ...
        # UPDATE UI PROPS FIRST - WebSpider 3D Paint pattern
...
classes = (
    WEBSPIDER_LAYERS_HT_header,
    WEBSPIDER_LAYERS_HT_footer,
