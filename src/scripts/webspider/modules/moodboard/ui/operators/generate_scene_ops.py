...
def _import_zip_scene(zip_bytes: bytes) -> bool:
    ...
    try:
        downloads_dir = os.path.join(os.path.expanduser("~"), "Downloads", "webspider3d_sam3d")
...
class WEBSPIDER_AI_OT_generate_scene(Operator):
    """Generate 3D scene from active segments using Scene Generation API"""
    bl_idname = "webspider_ai.generate_scene"
    bl_label = "Generate Scene"
    bl_options = {'REGISTER'}

    def execute(self, context):
        from webspider.modules.common.utils.image_utils import image_to_png_bytes, compress_image_for_upload
...
classes = (
    WEBSPIDER_AI_OT_generate_scene,
)
