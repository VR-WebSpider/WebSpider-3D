...
def _draw_drag_handle(layout):
    ...
    inner.operator(
        "webspider.agent_bubble_drag_history",
        text="━━━",
        emboss=True,
    )
...
class WEBSPIDER_AI_CHAT_UL_history(UIList):
    ...
    scene.webspider_chat_messages so every message is reachable; the
    ...
    bl_idname = "WEBSPIDER_AI_CHAT_UL_history"
...
def _draw_history(layout, scene, list_rows: int) -> None:
    ...
    template_list shows the entire scene.webspider_chat_messages collection
    ...
    if msg_count > 0:
        target = msg_count - 1
        if scene.webspider3d_bubble_history_active_index != target:
            scene.webspider3d_bubble_history_active_index = target

    layout.template_list(
        "WEBSPIDER_AI_CHAT_UL_history", "",
        scene, "webspider_chat_messages",
        scene, "webspider3d_bubble_history_active_index",
        rows=max(3, min(list_rows, 12)),
        type='DEFAULT',
    )


class WEBSPIDER_AI_CHAT_MT_agent_bubble(Menu):
    """Body of the floating agent bubble popup."""

    bl_idname = "WEBSPIDER_AI_CHAT_MT_agent_bubble"
    bl_label = "Agent"
    ...
    def draw(self, context):
        ...
        # the drag distance.
        history_lines = (
            getattr(scene, "webspider3d_bubble_history_lines", 0) if scene else 0
        )
        if history_lines > 0:
        ...
        # webspider_chat_input detects the \x1F submit marker and fires
        # the send operator).
        ...
        if scene is not None and hasattr(scene, "webspider_chat_mode"):
            action_row.prop(scene, "webspider_chat_mode", text="")
...
classes = (
    WEBSPIDER_AI_CHAT_UL_history,
    WEBSPIDER_AI_CHAT_MT_agent_bubble,
)
