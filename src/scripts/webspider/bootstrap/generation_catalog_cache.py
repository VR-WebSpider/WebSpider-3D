...
logout/unregister. Persistence uses Blender's per-user WebSpider 3D data directory,
with ``~/.webspider3d`` as a fallback.
"""

import threading
from typing import Any, Dict, List, Optional, Tuple

from webspider.bootstrap.generation_catalog import storage as generation_catalog_storage
from webspider.bootstrap.generation_catalog.queries import GenerationCatalogQueries
...
def refresh_generation_catalog_cache() -> None:
    ...
    threading.Thread(
        ...
        daemon=True,
        name="WebSpider 3DGenerationCatalogRefresh",
    ).start()
...
def _on_catalog_swapped() -> None:
    ...
    """
    try:
        from webspider.modules.common.generation_params.core.engine import (
...
def _start_background_fetch() -> None:
    ...
    threading.Thread(
        target=_fetch_data_sync, daemon=True, name="WebSpider 3DGenerationCatalogCache"
    ).start()
...
def _periodic_revalidate() -> float:
    ...
    if not busy:
        threading.Thread(
            target=_fetch_data_sync,
            daemon=True,
            name="WebSpider 3DGenerationCatalogRevalidate",
        ).start()
