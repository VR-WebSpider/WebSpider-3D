...
"""
WebSpider Chat Properties

Property definitions for the WebSpider Chat space.
"""
...
class WebSpiderAIChatAttachment(PropertyGroup):
...
def _deferred_submit_feedback(bubble_id):
    ...
    try:
        bpy.ops.webspider_ai_chat.submit_feedback_comment(bubble_id=bubble_id)
    except Exception:
        logger.warning(f"Failed to submit feedback comment for {bubble_id}", exc_info=True)


class WebSpiderAIChatMessage(PropertyGroup):
    ...
    attachments: CollectionProperty(
        type=WebSpiderAIChatAttachment,
        name="Attachments",
...
classes = (
    WebSpiderAIChatAttachment,
    WebSpiderAIChatMessage,
)
...
def on_chat_input_changed(self, context):
    ...
    UI_BUT_TEXTEDIT_UPDATE flag in SPACE_WEBSPIDER_CHAT. Shift+Enter inserts a
    ...
    """
    if self.webspider_chat_input.endswith("\x1F"):
        # Strip the submit marker and submit
        self.webspider_chat_input = self.webspider_chat_input.rstrip("\x1F")
    ...
    try:
        if not self.webspider_chat_input and self.webspider_chat_mention_show:
...
def _execute_send_message():
    ...
    try:
        if hasattr(bpy.ops.webspider_ai_chat, 'send_message'):
            bpy.ops.webspider_ai_chat.send_message()
    except Exception:
...
def on_quick_prompt_input_changed(self, context):
    ...
    """
    if hasattr(self, 'webspider_chat_quick_prompt_input'):
        quick_input = self.webspider_chat_quick_prompt_input
        if quick_input.endswith("\x1F"):
            # Strip submit marker
            self.webspider_chat_quick_prompt_input = quick_input.rstrip("\x1F")
...
def _execute_quick_prompt():
    ...
    try:
        ...
        bpy.ops.webspider_ai_chat.quick_prompt('EXEC_DEFAULT')
    except Exception:
        pass  # Operator may not be available
...
def _has_input_image(scene, context):
    ...
    try:
        if len(scene.webspider_chat_pending_attachments) > 0:
            return True
    except Exception:
        pass
    try:
...
def _has_prompt(scene, context):
    return bool((scene.webspider_chat_input or "").strip())
...
_GENERATE_TYPE_HINTS = {
    'depth_to_image': (
        "Enter a render prompt. WebSpider AI will use the current scene's depth "
        "as the guide.",
        _has_prompt,
    ),
    'pbr_gen': (
...
def _ask_model_choice(scene, service_key):
    ...
    intercepted locally by WEBSPIDER_AI_CHAT_OT_select_slot_action; ignoring the
...
def on_generate_type_changed(self, context):
    ...
    scene = context.scene
    gen_type = scene.webspider_chat_generate_type
    ...
    try:
        scene.webspider_chat_generate_model = ""
...
def register():
    ...
    # Register slot type dependencies first (WebSpiderAIChatMessage uses them in CollectionProperty)
    ...
    bpy.types.Scene.webspider_chat_input = StringProperty(
        name="Chat Input",
    ...
    bpy.types.Scene.webspider_chat_messages = CollectionProperty(
        type=WebSpiderAIChatMessage,
        name="Chat Messages",
    ...
    bpy.types.Scene.webspider_chat_pending_attachments = CollectionProperty(
        type=WebSpiderAIChatAttachment,
    ...
    bpy.types.Scene.webspider_chat_mode = EnumProperty(
        name="Chat Mode",
    ...
    bpy.types.Scene.webspider_chat_plan_enabled = BoolProperty(
        name="Plan Mode",
    ...
    bpy.types.Scene.webspider_chat_is_busy = BoolProperty(
        name="WebSpider AI Is Busy",
    ...
    bpy.types.Scene.webspider_chat_layout_epoch = IntProperty(
    ...
    # session. The C++ message renderer gates the "Hi I'm WebSpider AI"
    ...
    bpy.types.Scene.webspider_chat_user_has_engaged = BoolProperty(
    ...
    bpy.types.Scene.webspider_chat_state = EnumProperty(
        name="Session State",
    ...
    # block) keys off THIS rather than the live webspider_chat_mode
    ...
    bpy.types.Scene.webspider_chat_active_turn_mode = StringProperty(
    ...
    bpy.types.Scene.webspider_chat_generate_type = EnumProperty(
        name="Generate Type",
    ...
    bpy.types.Scene.webspider_chat_generate_model = StringProperty(
        name="Generate Model",
    ...
    bpy.types.Scene.webspider_chat_model = EnumProperty(
        name="Model",
    ...
    bpy.types.Scene.webspider_chat_user_id = StringProperty(
        name="User ID",
        description="User ID for WebSpider Chat login",
        default="",
        maxlen=256,
    ...
    bpy.types.WindowManager.webspider_chat_password = StringProperty(
        name="Password",
        description="Password for WebSpider Chat login",
        default="",
        maxlen=256,
        subtype='PASSWORD',
    )

    bpy.types.WindowManager.webspider_chat_is_logged_in = BoolProperty(
        name="Is Logged In",
        description="Whether user is currently logged in",
        default=False,
    )

    bpy.types.WindowManager.webspider_chat_is_logging_in = BoolProperty(
        name="Is Logging In",
        description="Whether login is in progress",
        default=False,
    )

    bpy.types.WindowManager.webspider_chat_login_error = StringProperty(
        name="Login Error",
    ...
    bpy.types.WindowManager.webspider_chat_session_expired = BoolProperty(
        name="Session Expired",
        description="Session expired — re-login required",
        default=False,
    )

    bpy.types.Scene.webspider_chat_credits = IntProperty(
        name="Credits",
    ...
    bpy.types.WindowManager.webspider_chat_quick_prompt_input = StringProperty(
        name="Quick Prompt",
    ...
    bpy.types.WindowManager.webspider_chat_quick_prompt_mode = EnumProperty(
    ...
    # catalog-driven items as scene.webspider_chat_generate_type so the two
    # enums always share identifiers (quick_prompt copies by identifier).
    bpy.types.WindowManager.webspider_chat_quick_prompt_generate_type = EnumProperty(
    ...
    bpy.types.WindowManager.webspider_ai_instance_id = StringProperty(
        name="Instance ID",
    ...
    bpy.types.Scene.webspider_ai_session_id = StringProperty(
        name="Session ID",
...
def unregister():
    ...
    for attr in (
        'webspider_chat_layout_epoch',
        'webspider_ai_session_id', 'webspider_chat_credits', 'webspider_chat_user_id',
        'webspider_chat_model', 'webspider_chat_generate_type',
        'webspider_chat_generate_model', 'webspider_chat_plan_enabled',
        'webspider_chat_is_busy', 'webspider_chat_state', 'webspider_chat_active_turn_mode',
        'webspider_chat_mode',
        'webspider_chat_pending_attachments', 'webspider_chat_messages', 'webspider_chat_input',
    ):
    ...
    for attr in (
        'webspider_ai_instance_id', 'webspider_chat_quick_prompt_generate_type',
        'webspider_chat_quick_prompt_mode', 'webspider_chat_quick_prompt_input',
        'webspider_chat_session_expired', 'webspider_chat_login_error',
        'webspider_chat_is_logging_in', 'webspider_chat_is_logged_in',
        'webspider_chat_password',
    ):
