 ...
 *     `scene.webspider_ai_chat_mention_query` property (webspider_ai_chat_mention_publish
 *     fires RNA_property_update, which runs the Python search callback
 *     synchronously — same bridge as the \x1F Enter-submit marker).
 *   - Python (space_webspider_ai_chat/core/mention_registry.py) matches the query
 *     against root objects / materials / assets and fills the
 *     `scene.webspider_ai_chat_mention_items` collection + `_active` + `_show`.
 ...
 * as webspider_ai_chat_is_busy in webspider_ai_chat_footer.cc).
 */
...
{
  PropertyRNA *prop = mention_prop_find(scene_ptr, "webspider_ai_chat_mention_items");
  if (!prop) {
    return 0;
  }
...
void webspider_ai_chat_mention_publish(bContext *C, Scene *scene, const char *query)
{
  ...
  PropertyRNA *prop = mention_prop_find(&scene_ptr, "webspider_ai_chat_mention_query");
  if (!prop) {
...
bool webspider_ai_chat_mention_is_open(Scene *scene)
{
  ...
  PointerRNA scene_ptr = RNA_id_pointer_create(&scene->id);
  PropertyRNA *show_prop = mention_prop_find(&scene_ptr, "webspider_ai_chat_mention_show");
...
{
  if (!webspider_ai_chat_mention_is_open(scene)) {
    return 0;
  }
  PointerRNA scene_ptr = RNA_id_pointer_create(&scene->id);
...
int webspider_ai_chat_mention_active_get(Scene *scene)
{
  ...
  PointerRNA scene_ptr = RNA_id_pointer_create(&scene->id);
  PropertyRNA *prop = mention_prop_find(&scene_ptr, "webspider_ai_chat_mention_active");
  if (!prop) {
    return 0;
  }
...
{
  const int count = webspider_ai_chat_mention_row_count(scene);
  if (count <= 0) {
    return;
  }
  const int active = webspider_ai_chat_mention_active_get(scene);
  webspider_ai_chat_mention_active_set(scene, (active + dir + count) % count);
}
...
void webspider_ai_chat_mention_dismiss(Scene *scene)
{
  ...
  PointerRNA scene_ptr = RNA_id_pointer_create(&scene->id);
  PropertyRNA *prop = mention_prop_find(&scene_ptr, "webspider_ai_chat_mention_show");
  if (prop) {
...
int webspider_ai_chat_mention_insert_text_get(Scene *scene, char *r_buf, int buf_maxncpy)
{
  ...
  PointerRNA scene_ptr = RNA_id_pointer_create(&scene->id);
  PropertyRNA *items_prop = mention_prop_find(&scene_ptr, "webspider_ai_chat_mention_items");
  if (!items_prop) {
    return 0;
  }
  ...
  const int active = std::clamp(webspider_ai_chat_mention_active_get(scene), 0, count - 1);
...
int webspider_ai_chat_mention_row_hit(Scene *scene, const ARegion *region, const int xy[2])
{
  ...
  const int count = webspider_ai_chat_mention_rows_get(scene, region->winx, nullptr, rows);
...
void webspider_ai_chat_mention_draw(Scene *scene, ARegion *region)
{
  rctf panel;
  rctf rows[FOOTER_MENTION_MAX_ROWS];
  const int count = webspider_ai_chat_mention_rows_get(scene, region->winx, &panel, rows);
  if (count <= 0) {
    return;
  }
