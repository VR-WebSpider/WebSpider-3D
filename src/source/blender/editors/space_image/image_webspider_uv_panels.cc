 ...
 * \ingroup spimage
 *
 * WebSpider 3D UV panels (Redo, Transform, Unwrap) for IMAGE_EDITOR sidebar.
 */
...
/* Forward declarations for helpers in image_webspider_uv_helpers.cc */
int webspider_uvedit_center(Scene *scene, const Span<Object *> objects, float center[2]);
bool webspider_uvedit_bounds(Scene *scene,
...
static void webspider_uv_redo_cb(bContext *C, void *arg_op, int /*arg_unused*/)
{
  ...
  ARegion *window_region = webspider_uv_find_window_region(C);
...
static bool webspider_uv_redo_panel_poll(const bContext *C, PanelType * /*pt*/)
{
  /* Only show in WEBSPIDER_UV mode */
  ...
  if (wm != nullptr) {
    ...
    PropertyRNA *prop = RNA_struct_find_property(&wm_ptr, "webspider_uv_ui");
    if (prop != nullptr) {
      PointerRNA webspider_uv_ui = RNA_pointer_get(&wm_ptr, "webspider_uv_ui");
      if (webspider_uv_ui.data != nullptr) {
        int active_panel = RNA_enum_get(&webspider_uv_ui, "active_panel");
...
static void webspider_uv_redo_panel_draw_header(const bContext *C, Panel *panel)
{
  wmOperator *op = WM_operator_last_redo(C);
  if (op != nullptr) {
...
static void webspider_uv_redo_panel_draw(const bContext *C, Panel *panel)
{
  wmOperator *op = WM_operator_last_redo(C);
  if (op == nullptr) {
    return;
  ...
  UI_block_func_handle_set(block, webspider_uv_redo_cb, op);
...
{
  PanelType *pt = MEM_callocN<PanelType>("webspider_uv_redo_panel");

  STRNCPY_UTF8(pt->idname, "WEBSPIDER_UV_PT_redo");
  STRNCPY_UTF8(pt->label, N_("Adjust Last Operation"));
  STRNCPY_UTF8(pt->translation_context, BLT_I18NCONTEXT_DEFAULT_BPYRNA);

  pt->draw_header = webspider_uv_redo_panel_draw_header;
  pt->draw = webspider_uv_redo_panel_draw;
  pt->poll = webspider_uv_redo_panel_poll;
...
static bool webspider_uv_transform_panel_poll(const bContext *C, PanelType * /*pt*/)
{
...
static void webspider_uv_transform_panel_draw(const bContext *C, Panel *panel)
{
  ...
  webspider_uv_pivot_point = sima->around;
   ...
   * when there are no bounds. Pivot stays interactive in both states. */
  {
    ...
    const bool has_selection = webspider_uvedit_center(scene, objects, center);

    float range_xy[2][2] = {{-10.0f, 10.0f}, {-10.0f, 10.0f}};
    if (has_selection) {
      copy_v2_v2(webspider_uv_vertex_old_center, center);

      CLAMP_MAX(range_xy[0][0], webspider_uv_vertex_old_center[0]);
      CLAMP_MIN(range_xy[0][1], webspider_uv_vertex_old_center[0]);
      CLAMP_MAX(range_xy[1][0], webspider_uv_vertex_old_center[1]);
      CLAMP_MIN(range_xy[1][1], webspider_uv_vertex_old_center[1]);

      if (!(sima->flag & SI_COORDFLOATS)) {
        webspider_uv_vertex_old_center[0] *= imx;
        webspider_uv_vertex_old_center[1] *= imy;
        mul_v2_fl(range_xy[0], imx);
        mul_v2_fl(range_xy[1], imy);
      }
    }
    else {
      webspider_uv_vertex_old_center[0] = 0.0f;
      webspider_uv_vertex_old_center[1] = 0.0f;
      webspider_uv_vertex_old_angle = 0.0f;
    }
    ...
    const bool has_bounds = webspider_uvedit_bounds(scene, objects, min_uv, max_uv);
    if (has_bounds) {
      webspider_uv_size_target[0] = max_uv[0] - min_uv[0];
      webspider_uv_size_target[1] = max_uv[1] - min_uv[1];
    }
    else {
      webspider_uv_size_target[0] = 0.0f;
      webspider_uv_size_target[1] = 0.0f;
    }
    ...
    UI_block_func_handle_set(move_block, do_webspider_uvedit_transform, nullptr);
    ...
    uiDefBut(move_block, ButType::Label, 0, IFACE_("X"),
             ...
             nullptr, 0.0f, 0.0f, "");
    but = uiDefButF(move_block, ButType::Num, B_WEBSPIDER_UVEDIT_VERTEX, "",
                     row_label_w + xy_label_w, y, xy_input_w, UI_UNIT_Y,
                     &webspider_uv_vertex_old_center[0], UNPACK2(range_xy[0]), "");
    ...
    but = uiDefButF(move_block, ButType::Num, B_WEBSPIDER_UVEDIT_VERTEX, "",
                     ...
                     &webspider_uv_vertex_old_center[1], UNPACK2(range_xy[1]), "");
    UI_but_number_step_size_set(but, step);
    UI_but_number_precision_set(but, digits);
    ...
    UI_block_func_handle_set(angle_block, do_webspider_uvedit_transform, nullptr);
    ...
    uiBut *angle_but = uiDefButF(angle_block, ButType::Num,
                                 B_WEBSPIDER_UVEDIT_ROTATE, "",
                                 row_label_w, -UI_UNIT_Y,
                                 angle_input_w, UI_UNIT_Y,
                                 &webspider_uv_vertex_old_angle, -360.0f, 360.0f,
    ...
    UI_block_func_handle_set(scale_block, do_webspider_uvedit_transform, nullptr);
    ...
    uiDefBut(scale_block, ButType::Label, 0, IFACE_("X"),
             row_label_w, y, xy_label_w, UI_UNIT_Y,
             nullptr, 0.0f, 0.0f, "");
    but = uiDefButF(scale_block, ButType::Num, B_WEBSPIDER_UVEDIT_SCALE, "",
                     row_label_w + xy_label_w, y, xy_input_w, UI_UNIT_Y,
                     &webspider_uv_size_target[0], 0.0f, 10.0f, "Width in UV space");
    ...
    but = uiDefButF(scale_block, ButType::Num, B_WEBSPIDER_UVEDIT_SCALE, "",
                     row_label_w + half_content + xy_gap + xy_label_w, y,
                     xy_input_w, UI_UNIT_Y,
                     &webspider_uv_size_target[1], 0.0f, 10.0f, "Height in UV space");
    UI_but_number_step_size_set(but, 1);
    UI_but_number_precision_set(but, 3);
  }
  ...
  /* ===== Move on Axis sub-section ===== */
  {
    ...
    if (ot_uv) {
      ...
      UI_block_func_handle_set(apply_move_block, do_webspider_uvedit_transform, nullptr);
      uiDefBut(apply_move_block, ButType::But, B_WEBSPIDER_UVEDIT_MOVE_AXIS,
  ...
  /* ===== 2D Cursor sub-section ===== */
  {
    ...
    if (sima->flag & SI_COORDFLOATS) {
      copy_v2_v2(webspider_uv_cursor_edit, sima->cursor);
    }
    else {
      webspider_uv_cursor_edit[0] = sima->cursor[0] * float(imx);
      webspider_uv_cursor_edit[1] = sima->cursor[1] * float(imy);
    }

    uiBlock *cursor_block = cursor_col->absolute_block();
    UI_block_func_handle_set(cursor_block, do_webspider_uvedit_transform, nullptr);
    ...
    but = uiDefButF(cursor_block, ButType::Num, B_WEBSPIDER_UVEDIT_CURSOR, "",
                     label_w, y, input_w, UI_UNIT_Y,
                     &webspider_uv_cursor_edit[0], -FLT_MAX, FLT_MAX, "Cursor X position");
    UI_but_number_step_size_set(but, step);
    UI_but_number_precision_set(but, digits);
    ...
    but = uiDefButF(cursor_block, ButType::Num, B_WEBSPIDER_UVEDIT_CURSOR, "",
                     ...
                     &webspider_uv_cursor_edit[1], -FLT_MAX, FLT_MAX, "Cursor Y position");
    UI_but_number_step_size_set(but, step);
    UI_but_number_precision_set(but, digits);
  }
  ...
  /* ===== Mirror sub-section ===== */
  {
    ...
    row->scale_y_set(1.3f);
    PointerRNA op_props_x = row->op("WEBSPIDER_OT_mirror", IFACE_("X Axis"), ICON_NONE);
    RNA_enum_set_identifier((bContext *)C, &op_props_x, "axis", "X");
    PointerRNA op_props_y = row->op("WEBSPIDER_OT_mirror", IFACE_("Y Axis"), ICON_NONE);
...
{
  PanelType *pt = MEM_callocN<PanelType>("webspider_uv_transform_panel");

  STRNCPY_UTF8(pt->idname, "WEBSPIDER_UV_PT_transform");
  STRNCPY_UTF8(pt->label, N_("Transform"));
  STRNCPY_UTF8(pt->translation_context, BLT_I18NCONTEXT_DEFAULT_BPYRNA);

  pt->draw = webspider_uv_transform_panel_draw;
  pt->poll = webspider_uv_transform_panel_poll;
  pt->space_type = SPACE_IMAGE;
  pt->region_type = RGN_TYPE_CHANNELS;
