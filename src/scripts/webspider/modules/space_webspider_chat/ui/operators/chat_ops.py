...
"""
WebSpider Chat Send Message Operator
...
def get_image_encoder():
    ...
    if _image_encoder_executor is None:
        _image_encoder_executor = ThreadPoolExecutor(
            max_workers=2,
            thread_name_prefix="webspider_ai_image_encoder"
        )
...
class WEBSPIDER_AI_CHAT_OT_send_message(Operator):
    """Send a chat message via HTTP/SSE"""
    bl_idname = "webspider_ai_chat.send_message"
    ...
    def execute(self, context):
        ...
        if not is_modify and not is_awaiting_input:
            stale_placeholders = []
            for i, m in enumerate(scene.webspider_chat_messages):
            ...
            for i in reversed(stale_placeholders):
                scene.webspider_chat_messages.remove(i)
        ...
        if not is_modify and not is_awaiting_input:
            placeholder = scene.webspider_chat_messages.add()
        ...
        if is_modify or is_awaiting_input:
            ...
            plan_required = getattr(scene, 'webspider_chat_plan_enabled', True)
