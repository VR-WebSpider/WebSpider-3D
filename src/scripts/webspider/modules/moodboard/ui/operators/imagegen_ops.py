...
def _get_imagegen_listener():
    ...
    - on_start: clear ``webspider_ai_imagegen_error``
    - on_finish: redraw WEBSPIDER_AI areas
    """
    ...
    def _on_start(scene):
        try:
            scene.webspider_ai_imagegen_error = ""
...
class WEBSPIDER_AI_OT_imagegen_generate(Operator):
    """Generate images using AI and add them to the moodboard"""

    bl_idname = "webspider_ai.imagegen_generate"
    ...
    def execute(self, context):
        ...
        if self.from_chat:
            prompt = getattr(scene, 'webspider_ai_imagegen_prompt', '')
        else:
            if hasattr(scene, 'webspider_ai_moodboard_sidebar'):
                ...
                if sidebar and hasattr(sidebar, 'tab_imagegen'):
                    ...
                    if sidebar_tab:
                        ...
                        if not sidebar_prompt or not sidebar_prompt.strip():
                            try:
                                sidebar_prompt = scene.path_resolve(
                                    "webspider_ai_moodboard_sidebar.tab_imagegen.prompt"
                                )
                            except Exception as e:
                        ...
                        if not sidebar_prompt or not sidebar_prompt.strip():
                            try:
                                sb = getattr(scene, 'webspider_ai_moodboard_sidebar', None)
                                if sb:
            ...
            if not prompt or not prompt.strip():
                global_prompt = getattr(scene, 'webspider_ai_imagegen_prompt', '')
        ...
        if self.from_chat:
            ...
            for ref_item in scene.webspider_ai_imagegen_ref_images:
                if ref_item.image:
                    try:
            ...
            if use_sidebar_props:
                ...
                model = scene.webspider_ai_imagegen_model
                    ...
                    if hasattr(sidebar_tab, 'reference_images'):
                        for ref_item in sidebar_tab.reference_images:
                            ...
                            if not img and ref_item.moodboard_index >= 0:
                                mb_images = scene.webspider_ai_moodboard_images
        ...
        if self.from_chat:
            ...
            style = scene.webspider_ai_imagegen_style
            aspect_ratio = scene.webspider_ai_imagegen_aspect_ratio
            resolution = getattr(scene, "webspider_ai_imagegen_resolution", "1K")
            negative_prompt = scene.webspider_ai_imagegen_negative_prompt or None

        stored_prompt = prompt.strip()
        stored_num_images = getattr(scene, "webspider_ai_imagegen_num_images", 1)
...
classes = (
    WEBSPIDER_AI_OT_imagegen_generate,
)
