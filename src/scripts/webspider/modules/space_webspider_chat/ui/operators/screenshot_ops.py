...
"""Screenshot capture operators for WebSpider Chat."""
...
from ...core.image_utils import get_webspider3d_screenshots_dir
from ...core.ui_utils import redraw_chat_areas

logger = get_logger(__name__)


class WEBSPIDER_AI_CHAT_OT_capture_screenshot(Operator):
    """Capture viewport screenshot and add as pending attachment"""
    bl_idname = "webspider_ai_chat.capture_screenshot"
    ...
    def execute(self, context):
        ...
        pending = scene.webspider_chat_pending_attachments
        ...
        # Create screenshot file inside WebSpider 3D's screenshots directory
        import glob
        screenshots_dir = get_webspider3d_screenshots_dir()
        timestamp = int(time.time() * 1000)  # Milliseconds for uniqueness
        screenshot_path = os.path.join(screenshots_dir, f"webspider_ai_screenshot_{os.getpid()}_{timestamp}.png")

        # Clean up old screenshot files from this Blender session (keep last 5)
        old_screenshots = sorted(glob.glob(os.path.join(screenshots_dir, f"webspider_ai_screenshot_{os.getpid()}_*.png")))
...
class WEBSPIDER_AI_CHAT_OT_send_with_screenshot(Operator):
    """Capture viewport screenshot and immediately send the message"""
    bl_idname = "webspider_ai_chat.send_with_screenshot"
    ...
    def execute(self, context):
        ...
        result = bpy.ops.webspider_ai_chat.capture_screenshot()
        if result != {'FINISHED'}:
            return result

        # Now send the message (which picks up pending attachments)
        return bpy.ops.webspider_ai_chat.send_message()


class WEBSPIDER_AI_CHAT_OT_snip_image(Operator):
    """Capture arbitrary screen region using system snipping tool"""
    bl_idname = "webspider_ai_chat.snip_image"
    ...
    def invoke(self, context, event):
        ...
        screenshots_dir = get_webspider3d_screenshots_dir()
        self._screenshot_path = os.path.join(screenshots_dir, f"webspider_ai_snip_{int(time.time())}.png")
    ...
    def modal(self, context, event):
        ...
        if self._process and self._process.poll() is not None:
            ...
            pending = context.scene.webspider_chat_pending_attachments
            attachment = pending.add()
...
def _tag_chat_redraw(context):
    """Trigger redraw on all WebSpider Chat areas."""
    redraw_chat_areas()


classes = (
    WEBSPIDER_AI_CHAT_OT_capture_screenshot,
    WEBSPIDER_AI_CHAT_OT_send_with_screenshot,
    WEBSPIDER_AI_CHAT_OT_snip_image,
)
