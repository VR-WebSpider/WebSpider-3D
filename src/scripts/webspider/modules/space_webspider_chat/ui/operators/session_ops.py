...
"""
Session management operators for WebSpider Chat.
...
class WEBSPIDER_AI_CHAT_OT_connect(Operator):
    """Connect to the WebSocket server"""
    bl_idname = "webspider_ai_chat.connect"
    bl_label = "Connect"
    bl_description = "Connect to the WebSpider AI agent server"
    bl_options = {'REGISTER'}

    @classmethod
    def poll(cls, context):
        # Must be logged in to connect
        if not context.window_manager or not context.window_manager.webspider_chat_is_logged_in:
            return False
...
class WEBSPIDER_AI_CHAT_OT_disconnect(Operator):
    """Disconnect from the WebSocket server"""
    bl_idname = "webspider_ai_chat.disconnect"
    bl_label = "Disconnect"
    bl_description = "Disconnect from the WebSpider AI agent server"
    bl_options = {'REGISTER'}
    ...
    def execute(self, context):
        ...
        if DEV_MODE:
            ...
            context.scene.webspider_chat_messages.clear()
...
class WEBSPIDER_AI_CHAT_OT_new_session(Operator):
    """Start a new chat session"""
    bl_idname = "webspider_ai_chat.new_session"
    bl_label = "New Chat"
    ...
    def execute(self, context):
        ...
        # Re-arm the "Hi I'm WebSpider AI" greeting for the fresh chat.
...
class WEBSPIDER_AI_CHAT_OT_dev_cycle_state(Operator):
    """Cycle through states for UI testing (Dev Mode only)"""
    bl_idname = "webspider_ai_chat.dev_cycle_state"
...
class WEBSPIDER_AI_CHAT_OT_abort_session(Operator):
    """Abort the current session."""

    bl_idname = "webspider_ai_chat.abort_session"
    bl_label = "Abort"
    ...
    def _finalize_loader_bubble(context) -> None:
        ...
        try:
            messages = context.scene.webspider_chat_messages
            to_remove = []
...
classes = (
    WEBSPIDER_AI_CHAT_OT_connect,
    WEBSPIDER_AI_CHAT_OT_disconnect,
    WEBSPIDER_AI_CHAT_OT_new_session,
    WEBSPIDER_AI_CHAT_OT_dev_cycle_state,
