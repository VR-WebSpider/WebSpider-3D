...
``webspider_ai.lookdev360_generate`` pipeline operator):

- ``webspider_ai.texture_edit_generate`` — Hunyuan texture edit: exports the
...
- ``webspider_ai.texture_gen_matgen`` — procedural material generation: submits
...
class WEBSPIDER_AI_OT_texture_edit_generate(Operator):
    """Edit the selected mesh's textures with Hunyuan Texture Edit"""

    bl_idname = "webspider_ai.texture_edit_generate"
    ...
    def execute(self, context):
        ...
        try:
            job = enqueue_generation(
                ...
                scene_flag="webspider_ai_lookdev360_is_generating",
            )
            if not job:
                self.report(
...
class WEBSPIDER_AI_OT_texture_gen_matgen(Operator):
    """Generate a procedural material from the prompt"""

    bl_idname = "webspider_ai.texture_gen_matgen"
    ...
    def execute(self, context):
        ...
        try:
            from webspider.modules.paint.procedural_materials.matgen_queue import (
                enqueue_matgen_job,
            )
...
classes = (
    WEBSPIDER_AI_OT_texture_edit_generate,
    WEBSPIDER_AI_OT_texture_gen_matgen,
)
