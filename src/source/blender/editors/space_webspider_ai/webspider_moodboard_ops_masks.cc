 ...
 * Reads lasso points from scene.webspider_ai_edit_tool_state.lasso_points collection.
 */
static wmOperatorStatus moodboard_generate_lasso_mask_exec(bContext *C, wmOperator *op)
{
  ...
  PointerRNA scene_ptr = RNA_id_pointer_create(&scene->id);
  PropertyRNA *state_prop = RNA_struct_find_property(&scene_ptr, "webspider_ai_edit_tool_state");
  if (!state_prop) {
...
void WEBSPIDER_AI_OT_moodboard_generate_box_mask(wmOperatorType *ot)
{
  ...
  ot->idname = "WEBSPIDER_AI_OT_moodboard_generate_box_mask";
  ot->description = "Generate a black and white box mask image (C++ accelerated)";

  ot->exec = blender::ed::webspider_ai::moodboard_generate_box_mask_exec;
...
void WEBSPIDER_AI_OT_moodboard_generate_lasso_mask(wmOperatorType *ot)
{
  ...
  ot->idname = "WEBSPIDER_AI_OT_moodboard_generate_lasso_mask";
  ot->description = "Generate a black and white lasso mask image (C++ accelerated). Reads points from scene.webspider_ai_edit_tool_state.lasso_points";

  ot->exec = blender::ed::webspider_ai::moodboard_generate_lasso_mask_exec;
  ot->poll = blender::ed::webspider_ai::moodboard_poll;
