...
_BUG_REPORT_URL = "https://www.webspider3d.com/bug-report"
...
class WEBSPIDER_AI_UL_unified_queue(UIList):
    ...
    bl_idname = "WEBSPIDER_AI_UL_unified_queue"
    ...
    def draw_item(self, context, layout, data, item, icon, active_data,
        ...
        if not is_terminal:
            op = top.operator(
                "webspider_ai.queue_cancel_job", text="", icon='X', emboss=False,
            )
        ...
        if is_failed and (item.user_message or item.error):
            ...
            op = actions.operator(
                "webspider_ai.queue_copy_error", text="", icon='COPYDOWN',
                emboss=False,
            )
            op.feature_key = item.feature_key
            op.job_id = item.job_id
...
classes = (WEBSPIDER_AI_UL_unified_queue,)
...
def draw_queue_generate_footer(
    ...
    if cooldown:
        sub.operator("webspider_ai.hunyuan_generate", text="Added to queue!", icon='CHECKMARK')
    else:
        op = sub.operator("webspider_ai.hunyuan_generate", text="Generate", icon='PLAY')
...
def draw_unified_queue_panel(layout, context):
    ...
    if not hasattr(wm, "webspider_ai_queue"):
    ...
    layout.template_list(
        WEBSPIDER_AI_UL_unified_queue.bl_idname, "",
        pg, "items",
        pg, "active_index",
