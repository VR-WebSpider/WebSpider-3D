...
user's tempdir and are regenerated once per WebSpider 3D boot.
...
from webspider.modules.common.job_queue.core.job import JobState, RUNNING_STATES
...
# from a previous WebSpider 3D boot get regenerated at the new size.
_ICON_VERSION = "v2"
...
def _write_png(filepath: str, color, filled: bool) -> bool:
    tmp_name = "_webspider3d_queue_dot_tmp"
...
def _build_previews() -> None:
    ...
    if _pcoll is not None:
        return
    try:
        cache_dir = os.path.join(tempfile.gettempdir(), "webspider3d_queue_dots")
    ...
    try:
        ...
        for window in wm.windows:
            for area in window.screen.areas:
                if area.type in ('WEBSPIDER_AI', 'VIEW_3D'):
                    area.tag_redraw()
    except Exception:
        pass
...
def unregister() -> None:
    """Tear down the previews collection on WebSpider 3D shutdown."""
