...
float webspider_ai_chat_feedback_comment_input_height(PointerRNA *msg_ptr, const float input_width)
{
...
/* Diagnostics for the star row (set WEBSPIDER_FEEDBACK_DEBUG=1 and run from a
...
{
  static const bool enabled = getenv("WEBSPIDER_FEEDBACK_DEBUG") != nullptr;
  return enabled;
}
                                ...
                                const MessageLayoutData &layout)
{
   ...
   * webspider_ai_chat_build_layout_cache(). */
  ...
  for (const MessageLayoutData &layout : layout_cache) {
    ...
    for (int i = 0; i < FEEDBACK_STAR_COUNT; i++) {
      ...
      if (has_bounds && BLI_rctf_isect_pt(&star.bounds, view_x, view_y)) {
        wmOperatorType *ot = WM_operatortype_find("webspider_ai_chat.set_feedback_rating", true);
        if (ot) {
          PointerRNA op_ptr;
    ...
    {
      wmOperatorType *ot = WM_operatortype_find("webspider_ai_chat.toggle_feedback_comment", true);
      if (ot) {
        PointerRNA op_ptr;
        WM_operator_properties_create_ptr(&op_ptr, ot);
        RNA_string_set(&op_ptr, "bubble_id", layout.bubble_id);
