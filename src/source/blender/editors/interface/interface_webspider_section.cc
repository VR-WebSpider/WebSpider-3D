 ...
 * \ingroup edinterface
 *
 * WebSpider 3D custom UI widgets — helpers for styled layouts and controls.
 */
...
uiLayout *UI_layout_webspider_section(uiLayout *layout)
{
  ...
  for (int i = int(block->buttons.size()) - 1; i >= 0; i--) {
    ...
    if (but->type == ButType::Roundbox) {
      but->flag2 |= UI_BUT2_WEBSPIDER_SECTION;
      break;
    }
  }
...
void UI_layout_webspider_mark_last_dropdown(uiLayout *layout)
{
  uiBlock *block = layout->block();
  ...
  for (int i = int(block->buttons.size()) - 1; i >= 0; i--) {
    ...
    if (ELEM(but->type, ButType::Menu, ButType::Block, ButType::Popover)) {
      but->flag2 |= UI_BUT2_WEBSPIDER_DROPDOWN;
      break;
    }
  }
}

void UI_layout_webspider_mark_last_action(uiLayout *layout)
{
  uiBlock *block = layout->block();
  ...
  for (int i = int(block->buttons.size()) - 1; i >= 0; i--) {
    ...
    if (but->type == ButType::But) {
      but->flag2 |= UI_BUT2_WEBSPIDER_ACTION;
      break;
    }
  }
}

void UI_layout_webspider_mark_last_toggle(uiLayout *layout)
{
  uiBlock *block = layout->block();
  ...
  for (int i = int(block->buttons.size()) - 1; i >= 0; i--) {
    ...
    if (ELEM(but->type, ButType::Checkbox, ButType::CheckboxN)) {
      but->flag2 |= UI_BUT2_WEBSPIDER_TOGGLE;
      break;
    }
  }
}

void UI_layout_webspider_mark_last_input(uiLayout *layout)
{
  uiBlock *block = layout->block();
  ...
  for (int i = int(block->buttons.size()) - 1; i >= 0; i--) {
    ...
    if (but->type == ButType::Text) {
      but->flag2 |= UI_BUT2_WEBSPIDER_INPUT;
      break;
    }
  }
}
...
/* -------------------------------------------------------------------- */
/* WebSpider 3D Custom Panel Category Tabs                                      */
...
/* Padding constants */
#define WEBSPIDER_TAB_PAD_TEXT 8.0f
#define WEBSPIDER_TAB_PAD_BETWEEN 6.0f
...
void UI_panel_category_draw_all_webspider(ARegion *region, const char *category_id_active)
{
  ...
  /* Read all colors from the theme (space_webspider_ai in bTheme). */
  const bTheme *btheme = UI_GetTheme();
  const ThemeSpace *ts = &btheme->space_webspider_ai;
  ...
  ubyte4_to_float4(col_accent, ts->webspider_tab_accent);
  ubyte4_to_float4(col_strip_bg, ts->webspider_tab_strip_bg);
  ubyte4_to_float4(col_inactive, ts->webspider_tab_inactive);
  ubyte4_to_float4(col_text_active, ts->webspider_tab_text_active);
  ubyte4_to_float4(col_text_inactive, ts->webspider_tab_text_inactive);
  ubyte4_to_float4(col_glow, ts->webspider_tab_glow);
  ubyte4_to_float4(col_highlight, ts->webspider_tab_highlight);
  ubyte4_to_float4(col_indicator, ts->webspider_tab_indicator);
  ...
  const int tab_v_pad_text = round_fl_to_int(WEBSPIDER_TAB_PAD_TEXT * dpi_fac * zoom) + 2 * px;
  const int tab_v_pad = round_fl_to_int(WEBSPIDER_TAB_PAD_BETWEEN * dpi_fac * zoom);
...
#undef WEBSPIDER_TAB_PAD_TEXT
#undef WEBSPIDER_TAB_PAD_BETWEEN
