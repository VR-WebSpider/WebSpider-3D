...
``webspider.bootstrap.generation_catalog_cache`` — kept here (not in the
...
def _memoize(kind: str, key: str, builder):
    ...
    """
    try:
        from webspider.bootstrap.generation_catalog_cache import memoize_enum_items
...
def _dropdown(layout, data, prop, text):
    """Enum prop via the WebSpider 3D styled dropdown when available."""
