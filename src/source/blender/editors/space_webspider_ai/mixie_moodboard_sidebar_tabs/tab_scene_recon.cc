...
#include "../webspider_ai_moodboard_sidebar_constants.hh"
#include "../webspider_ai_moodboard_sidebar_intern.hh"
#include "../webspider_ai_moodboard_sidebar_types.hh"

namespace blender::ed::webspider_ai::moodboard::sidebar {
...
static bool get_tab_properties(Scene *scene, PointerRNA &tab_ptr)
{
  ...
  PropertyRNA *sidebar_prop = RNA_struct_find_property(&scene_ptr, "webspider_ai_moodboard_sidebar");
  if (!sidebar_prop) {
    return false;
  }
                                         ...
                                         int width)
{
  ...
  PointerRNA scene_ptr = RNA_id_pointer_create(&scene->id);
  PropertyRNA *mb_images_prop = RNA_struct_find_property(&scene_ptr, "webspider_ai_moodboard_images");
  if (!mb_images_prop) {
    return 0;
  }
...
void draw_tab_scene_recon(const bContext *C, uiBlock *block, const TabContentLayout &layout)
{
  ...
  PointerRNA scene_ptr = RNA_id_pointer_create(&scene->id);
  PropertyRNA *gen_prop = RNA_struct_find_property(&scene_ptr, "webspider_ai_scene_recon_is_generating");
  if (gen_prop) {
