...
class MBakeInfoOtherObject(bpy.types.PropertyGroup):
    object: PointerProperty(type=bpy.types.Object)
    object_name: StringProperty(default="")


class MBakeInfoSelectedVertex(bpy.types.PropertyGroup):
    index: IntProperty(default=0)


class MBakeInfoSelectedObject(bpy.types.PropertyGroup):
    object: PointerProperty(type=bpy.types.Object)
    object_name: StringProperty(default="")

    selected_vertex_indices: CollectionProperty(type=MBakeInfoSelectedVertex)


class MBakeInfoProps(bpy.types.PropertyGroup):
    is_baked: BoolProperty(
        default=False
    )  # Flag to mark if the image is from baking or not
    is_baked_channel: BoolProperty(
        default=False
    )  # Flag to mark if the image baked from main channel
    is_baked_entity: BoolProperty(
        default=False
    )  # Flag to mark if the image baked from entity
    ...
    baked_entity_type: StringProperty(
        name="Baked Entity Type", description="Baked entity type", default=""
    )
    ...
    blur: BoolProperty(
        name="Use Blur", description="Use blur to baked image", default=False
    )
    ...
    other_objects: CollectionProperty(type=MBakeInfoOtherObject)
    ...
    selected_objects: CollectionProperty(type=MBakeInfoSelectedObject)
...
classes = (
    MBakeInfoSelectedVertex,
    MBakeInfoSelectedObject,
    MBakeInfoOtherObject,
    MBakeInfoProps,
)


def register():
    """Register bake info properties.
    ...
    # Register m_bake_info property on Image datablock
    if not hasattr(bpy.types.Image, 'm_bake_info'):
        bpy.types.Image.m_bake_info = PointerProperty(type=MBakeInfoProps)


def unregister():
    """Unregister bake info properties"""
    # Unregister Image property
    if hasattr(bpy.types.Image, 'm_bake_info'):
        del bpy.types.Image.m_bake_info
