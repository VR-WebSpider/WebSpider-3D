...
"""
Space WebSpider Chat Module Constants

Centralized configuration values for the WebSpider Chat module.
"""
...
#   for normal / sandbox mode. It intentionally matches NO scene.webspider_ai_session_id
...
#   main scene webspider_ai_session_id (a UUID v4 set by SessionManager.start_session)
...
def is_lane_scene(scene) -> bool:
    ...
    return getattr(scene, 'webspider_ai_session_id', '').startswith(AGENT_LANE_SESSION_PREFIX)
...
# EnumProperty items for scene.webspider_chat_state
...
# Values are mirrored in C++ (webspider_chat_feedback.cc) — keep in sync.
...
# Project rules (scene.webspider_chat_rules) — persisted in the file and
...
# the C++ overlay (webspider_chat_rules_intern.hh / webspider_chat_layout_data.hh).
...
# webspider_chat_footer_constants.hh — the C++ dropdown never draws more rows.
MENTION_MAX_ITEMS = 6

# Max published query length in bytes (leading '@' included). Must match
# MENTION_QUERY_MAX in webspider_chat_footer_constants.hh.
MENTION_QUERY_MAXLEN = 96

# Max replacement text length. The C++ accept path reads insert_text into a
# 320-byte buffer (WEBSPIDER_AI_MENTION_INSERT_SIZE) and refuses longer values.
...
# Per-session cap on image files copied into ~/.webspider3d/chat_media/<session>/.
