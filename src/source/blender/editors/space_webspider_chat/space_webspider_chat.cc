 ...
 *
 * WebSpider Chat Space - Chat interface for WebSpider 3D.
...
#include "webspider_ai_chat_footer_intern.hh"

/* -------------------------------------------------------------------- */
/** \name Operators
 * \{ */

static void webspider_ai_chat_operatortypes()
{
  WM_operatortype_append(WEBSPIDER_AI_CHAT_OT_select_text);
  WM_operatortype_append(WEBSPIDER_AI_CHAT_OT_copy);
  WM_operatortype_append(WEBSPIDER_AI_CHAT_OT_drop_image);
  WM_operatortype_append(WEBSPIDER_AI_CHAT_OT_agent_bubble_show);
}
...
static void webspider_ai_chat_keymap(wmKeyConfig *keyconf)
{
  wmKeyMap *keymap = WM_keymap_ensure(keyconf, "WebSpider Chat", SPACE_WEBSPIDER_CHAT, RGN_TYPE_WINDOW);
  ...
  WM_keymap_add_item(keymap, "WEBSPIDER_AI_CHAT_OT_select_text", &select_params);
...
#endif
  WM_keymap_add_item(keymap, "WEBSPIDER_AI_CHAT_OT_copy", &copy_params);

  /* Ctrl+V / Cmd+V: paste image or text into chat.
   * Added to the main "WebSpider Chat" keymap (not a separate footer keymap)
...
#endif
  WM_keymap_add_item(keymap, "WEBSPIDER_AI_CHAT_OT_paste_image", &paste_params);
}
...
static SpaceLink *webspider_ai_chat_create(const ScrArea * /*area*/, const Scene * /*scene*/)
{
  ARegion *region;
  SpaceWebSpider AIChat *swebspider_ai_chat;

  swebspider_ai_chat = MEM_callocN<SpaceWebSpider AIChat>("initwebspider_aichat");
  swebspider_ai_chat->spacetype = SPACE_WEBSPIDER_CHAT;

  /* Initialize selection state */
  swebspider_ai_chat->sel_message_index = -1;
  swebspider_ai_chat->sel_start = 0;
  swebspider_ai_chat->sel_end = 0;

  /* Allocate per-instance runtime */
  swebspider_ai_chat->runtime = nullptr;  /* Lazily allocated on first use */

  /* Header region */
  region = BKE_area_region_new();
  BLI_addtail(&swebspider_ai_chat->regionbase, region);
  ...
  return (SpaceLink *)swebspider_ai_chat;
}

static void webspider_ai_chat_free(SpaceLink *sl)
{
  SpaceWebSpider AIChat *swebspider_ai_chat = (SpaceWebSpider AIChat *)sl;

  /* Free per-instance runtime data (layout cache, empty prompts, etc.) */
  webspider_ai_chat_free_runtime(swebspider_ai_chat);

  /* Clear footer cache (declared in webspider_ai_chat_footer_cache.cc) */
  footer_cache_clear();

  /* Clear property caches (declared in webspider_ai_chat_props.cc) */
  webspider_ai_chat_clear_property_caches();
}

static void webspider_ai_chat_init(wmWindowManager * /*wm*/, ScrArea *area)
{
...
static SpaceLink *webspider_ai_chat_duplicate(SpaceLink *sl)
{
  SpaceWebSpider AIChat *swebspider_ai_chat_new = static_cast<SpaceWebSpider AIChat *>(MEM_dupallocN(sl));
  /* New instance gets its own fresh runtime — don't share with source */
  swebspider_ai_chat_new->runtime = nullptr;
  return (SpaceLink *)swebspider_ai_chat_new;
}
...
static void webspider_ai_chat_header_listener(const wmRegionListenerParams *params)
{
  ...
  switch (wmn->category) {
    case NC_SCREEN:
      ...
      break;
    case NC_SPACE:
      if (wmn->data == ND_SPACE_WEBSPIDER_CHAT) {
        ED_region_tag_redraw(region);
      }
      break;
  }
...
static void webspider_ai_chat_footer_region_listener(const wmRegionListenerParams *params)
{
  ARegion *region = params->region;
  const wmNotifier *wmn = params->notifier;
...
static void webspider_ai_chat_space_listener(const wmSpaceTypeListenerParams *params)
{
  ScrArea *area = params->area;
  const wmNotifier *wmn = params->notifier;
...
static void webspider_ai_chat_space_blend_write(BlendWriter *writer, SpaceLink *sl)
{
  SpaceWebSpider AIChat *swebspider_ai = (SpaceWebSpider AIChat *)sl;
  /* Don't save runtime pointer — it's regenerated on load */
  void *runtime_backup = swebspider_ai->runtime;
  swebspider_ai->runtime = nullptr;
  BLO_write_struct(writer, SpaceWebSpider AIChat, sl);
  swebspider_ai->runtime = runtime_backup;
}
...
void ED_spacetype_webspider_ai_chat()
{
  std::unique_ptr<SpaceType> st = std::make_unique<SpaceType>();
  ARegionType *art;

  st->spaceid = SPACE_WEBSPIDER_CHAT;
  STRNCPY_UTF8(st->name, "WebSpider AIChat");
  st->iconid = ICON_CONSOLE;

  st->create = webspider_ai_chat_create;
  st->free = webspider_ai_chat_free;
  st->init = webspider_ai_chat_init;
  st->duplicate = webspider_ai_chat_duplicate;
  st->operatortypes = webspider_ai_chat_operatortypes;
  st->keymap = webspider_ai_chat_keymap;
  st->dropboxes = webspider_ai_chat_dropboxes;
  st->blend_write = webspider_ai_chat_space_blend_write;
  st->listener = webspider_ai_chat_space_listener;

  /* regions: main window (custom chat drawing) */
  art = MEM_callocN<ARegionType>("spacetype webspider_ai_chat region");
  ...
  art->layout = webspider_ai_chat_main_region_layout;  /* Update View2D when footer resizes */
  art->draw = webspider_ai_chat_main_region_draw;
  ...
  art->cursor = webspider_ai_chat_main_region_cursor;  /* Hover tracking for option bubbles */
  ...
  /* regions: header */
  art = MEM_callocN<ARegionType>("spacetype webspider_ai_chat header region");
  art->regionid = RGN_TYPE_HEADER;
  art->prefsizey = HEADERY;

  art->keymapflag = ED_KEYMAP_UI | ED_KEYMAP_HEADER;
  art->listener = webspider_ai_chat_header_listener;
  art->init = webspider_ai_chat_header_region_init;
  art->draw = webspider_ai_chat_header_region_draw;

  BLI_addhead(&st->regiontypes, art);

  /* regions: footer (implemented as TOOLS region for dynamic sizing) */
  art = MEM_callocN<ARegionType>("spacetype webspider_ai_chat footer region");
  ...
  art->listener = webspider_ai_chat_footer_region_listener;
  art->init = webspider_ai_chat_footer_region_init;
  art->layout = webspider_ai_chat_footer_region_layout;  /* Dynamic sizing for pending attachments */
  art->draw = webspider_ai_chat_footer_region_draw;
