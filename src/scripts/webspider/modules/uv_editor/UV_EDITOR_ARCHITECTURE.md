...
# WebSpider 3D UV Editor & UV Properties - Technical Architecture

This document provides comprehensive technical documentation for the WebSpider 3D UV Editor and UV Properties implementation. It is designed for developers extending the UV Editor and as context for LLMs adding new features.
...
The WebSpider 3D UV Editor uses a **dual-space architecture**:

1. **IMAGE_EDITOR** (mode: `WEBSPIDER_UV`) - The main UV editing viewport
2. **WEBSPIDER_UV_PROPERTIES** - A separate properties panel space

```
┌─────────────────────────────────────┬──────────────────────┐
│                                     │                      │
│         IMAGE_EDITOR                │  WEBSPIDER_UV_PROPERTIES │
│         (WEBSPIDER_UV mode)             │                      │
│                                     │  ┌────────────────┐  │
...
1. **Header-Based** (Most panels)
   ...
   - Controlled by `webspider3d_uv_ui.active_panel` enum
   - Python panels
...
Since UV operations require IMAGE_EDITOR context but panels draw in WEBSPIDER_UV_PROPERTIES space, all operations must **switch context** before execution:

```
WEBSPIDER_UV_PROPERTIES → Save Context → Switch to IMAGE_EDITOR → Execute → Restore Context
```
...
```
src/scripts/webspider/modules/uv_editor/
├── common/
│   └── uv_utils.py              # Common utilities, decorators, and poll functions
└── ui/
    ├── properties.py            # UI state management (WebSpider 3DUVUIState)
    ├── base/
    │   ├── __init__.py
...
```
src/source/blender/editors/space_webspider3d_uv_properties/
├── space_webspider3d_uv_properties.cc   # Main C implementation
...
| `properties.py`                | Defines `WebSpider 3DUVUIState` PropertyGroup with `active_panel` enum and expansion states    |
...
| `space_webspider3d_uv_properties.cc` | C panels: Transform (Move/Resize/Cursor/Arrange), Redo panel, Unwrap                   |
...
Located in `space_image.py` (lines 750-762), the panel selector appears when `sima.mode == 'WEBSPIDER_UV'`:
...
if sima.mode == 'WEBSPIDER_UV':
    ...
    if hasattr(wm, 'webspider3d_uv_ui'):
        ...
        row.scale_y = 1.2
        row.prop_enum(wm.webspider3d_uv_ui, "active_panel", 'TRANSFORM', text="Transform", icon='ORIENTATION_GLOBAL')
        row.prop_enum(wm.webspider3d_uv_ui, "active_panel", 'PACK_ISLANDS', text="Pack", icon='PACKAGE')
        # ... more buttons
```
...
class WEBSPIDER_UV_PT_pack_islands(Panel):
    bl_space_type = 'WEBSPIDER_UV_PROPERTIES'
    ...
    def poll(cls, context):
        ...
        if not hasattr(wm, 'webspider3d_uv_ui'):
            return False
        return wm.webspider3d_uv_ui.active_panel == 'PACK_ISLANDS'
```
...
```cpp
static bool webspider3d_uv_transform_panel_poll(const bContext *C, PanelType * /*pt*/)
{
  ...
  LISTBASE_FOREACH (ScrArea *, area, &screen->areabase) {
    if (area->spacetype == SPACE_IMAGE) {
      ...
      if (sima && sima->mode == SI_MODE_WEBSPIDER_UV) {
...
| ------------------------------- | --------------- | -------------------------------------------------------------------------- | ------ |
| `WEBSPIDER_UV_PT_selection`         | Tool: Selection | Selection tools and operations                                             | Active |
| `WEBSPIDER_UV_PT_transform_options` | ~~`TRANSFORM`~~ | ~~Mirror, Proportional Editing, Round to Pixels, Align, Align Rotation~~  | **DEPRECATED - Replaced by C panel** |
| `WEBSPIDER_UV_PT_pack_islands`      | `PACK_ISLANDS`  | Layout panel: Pack Islands props sub-section + Custom Region sub-section (when Pack To = Custom Region; appears between props and the Pack Islands run button) + full-width Pack Islands button + Average Islands Scale sub-section (with Non-Uniform / Shear) + Minimize Stretch sub-section | Active |
| `WEBSPIDER_UV_PT_uv_set`            | `UV_SET`        | UV Maps, Active Image, UDIM Tiles, Grid, Stretch, Display                  | Active |
| `WEBSPIDER_UV_PT_texel_density`     | `TEXEL_DENSITY` | Texel density calculation and application                                  | Active |
| `WEBSPIDER_UV_PT_unwrap`            | `UNWRAP`        | Combined panel with two sub-sections: **Unwrap** (method dropdown + properties + Unwrap button) and **Project** (type dropdown + properties + Project button) | Active |
| `WEBSPIDER_UV_PT_snapping`          | `SNAPPING`      | Snap settings and operations                                               | Active |
| `WEBSPIDER_UV_PT_unwrap` (Project section) | `UNWRAP` | Project sub-section inside the Unwrap panel: Type dropdown (Cube, Cylinder, Sphere, Camera, Normal, Planar) + type-specific properties + Project button | Active |
| `WEBSPIDER_UV_PT_tools`             | Tool: UV Sculpt | UV tool activation buttons and properties (Grab, Relax, Pinch, Rip Region) | Active |
| `WEBSPIDER_UV_PT_functions`         | Tool: Functions | UV functions (Seam, Pin, Merge, Split, Hide)                               | Active |
| `WEBSPIDER_UV_PT_image`             | `IMAGE`         | Image operations (New, Open, Save, Transform, Pack/Unpack)                 | Active |
| `WEBSPIDER_UV_PT_export`            | `EXPORT`        | UV layout export settings and export button                                | Active |

> **Note:** `WEBSPIDER_UV_PT_transform_options` is commented out in the registration (panels.py:1864) and replaced by the C-based Transform panel.

### C Panels Overview

| Panel ID                | Active When        | Content                                                                                          | Implementation |
| ----------------------- | ------------------ | ------------------------------------------------------------------------------------------------ | -------------- |
| `WEBSPIDER_UV_PT_transform` | Tool: Transform    | Move (X/Y), Pivot, Rotation, Resize, 2D Cursor, Arrange Islands, Snapping, Mirror, Proportional Editing, Round to Pixels, Align, Align Rotation | C (space_webspider3d_uv_properties.cc) |
| `WEBSPIDER_UV_PT_redo`      | Tool: Transform    | Last operation properties (like Blender's Adjust Last Operation)                                 | C (space_webspider3d_uv_properties.cc) |
...
static float webspider3d_uv_vertex_old_center[2];        // Translation
static float webspider3d_uv_vertex_old_angle = 0.0f;     // Rotation angle
static float webspider3d_uv_vertex_applied_angle = 0.0f; // Cumulative rotation for delta
static float webspider3d_uv_size_target[2];              // Target dimensions for resize
static int webspider3d_uv_pivot_point;                   // Pivot mode (V3D_AROUND_*)
static float webspider3d_uv_cursor_edit[2];              // 2D cursor position
```
...
static void do_webspider3d_uvedit_transform(bContext *C, void * /*arg*/, int event) {
    // Guard clause - only handle known events
    if (event != B_WEBSPIDER_UVEDIT_VERTEX && event != B_WEBSPIDER_UVEDIT_ROTATE &&
        event != B_WEBSPIDER_UVEDIT_SCALE && event != B_WEBSPIDER_UVEDIT_CURSOR) {
        return;
    }
    ...
    if (event == B_WEBSPIDER_UVEDIT_VERTEX) {
        // Calculate delta and translate
        webspider3d_uvedit_translate(scene, objects, delta);
    }
    else if (event == B_WEBSPIDER_UVEDIT_ROTATE) {
        // Calculate pivot center and rotate
        webspider3d_uvedit_rotate(scene, objects, pivot_center, angle_rad);
    }
    // ... more events
...
```cpp
static void webspider3d_uv_transform_panel_draw(const bContext *C, Panel *panel) {
    ...
    // Create sub-panel
    PanelLayout move_layout = panel->layout->panel(C, "WEBSPIDER_UV_move_panel", false);
    move_layout.header->label(IFACE_("Move"), ICON_NONE);

    if (move_layout.body) {
        uiBlock *block = move_layout.body->absolute_block();
        UI_block_func_handle_set(block, do_webspider3d_uvedit_transform, nullptr);

        // Create buttons with event IDs
        uiDefButF(block, ButType::Num, B_WEBSPIDER_UVEDIT_VERTEX, IFACE_("X:"),
                  0, y -= UI_UNIT_Y, 200, UI_UNIT_Y,
                  &webspider3d_uv_vertex_old_center[0], min, max, "");
    }
...
UV operators require IMAGE_EDITOR context, but when called from WEBSPIDER_UV_PROPERTIES space, they fail. Wrappers solve this with `context.temp_override()`.
...
#### Context Getters
- `get_webspider3d_uv_image_editor(context)` - Find IMAGE_EDITOR in WEBSPIDER_UV mode
- `get_window_region(area)` - Get WINDOW region from area

#### Poll Functions
- `poll_webspider3d_uv_mode(context)` - Basic UV mode check
- `poll_webspider3d_uv_edit_mode(context)` - UV mode + edit mode mesh
- `poll_webspider3d_uv_with_image(context)` - UV mode with image
- `poll_webspider3d_uv_with_packed_image(context)` - UV mode with packed image
- `poll_webspider3d_uv_with_unpacked_image(context)` - UV mode with unpacked image
...
class WEBSPIDER_OT_snap_selected(Operator):
    @classmethod
    def poll(cls, context):
        # Manual poll - AVOID, use poll_webspider3d_uv_mode() instead
        for area in context.screen.areas:
            if area.type == 'IMAGE_EDITOR':
                sima = area.spaces.active
                if sima and sima.mode == 'WEBSPIDER_UV':
...
| -------------------------------- | -------------------------- | ------------------------------ |
| `webspider3d.open_uv_properties`       | Area split                 | Opens UV Properties panel      |
| `webspider3d.close_uv_properties`      | Area close                 | Closes UV Properties panel     |
| `webspider3d.snap_selected`            | `uv.snap_selected`         | Snap selection to target       |
| `webspider3d.snap_cursor`              | `uv.snap_cursor`           | Snap cursor to target          |
| `webspider3d.mirror`                   | `transform.mirror`         | Mirror UVs on axis             |
| `webspider3d.align`                    | `uv.align`                 | Align UVs                      |
| `webspider3d.align_rotation`           | `uv.align_rotation`        | Align island rotation          |
| `webspider3d.activate_uv_sculpt_grab`  | `wm.tool_set_by_id`        | Activates UV Sculpt Grab tool  |
| `webspider3d.activate_uv_sculpt_relax` | `wm.tool_set_by_id`        | Activates UV Sculpt Relax tool |
| `webspider3d.activate_uv_sculpt_pinch` | `wm.tool_set_by_id`        | Activates UV Sculpt Pinch tool |
| `webspider3d.activate_rip_region`      | `wm.tool_set_by_id`        | Activates Rip Region tool      |
| `webspider3d.average_islands_scale`    | `uv.average_islands_scale` | Average the size of UV islands       |
| `webspider3d.minimize_stretch`         | `uv.minimize_stretch`      | Minimize UV stretch                  |
| `webspider3d.custom_region_set`        | `uv.custom_region_set`     | Create custom regions for UV packing |
| `webspider3d.mark_seam`                | `uv.mark_seam`             | Mark selected edges as seam          |
| `webspider3d.clear_seam`               | `uv.mark_seam`             | Clear seam from selected edges       |
| `webspider3d.seams_from_islands`       | `uv.seams_from_islands`    | Set seams based on UV islands        |
| `webspider3d.stitch`                   | `uv.stitch`                | Stitch UV vertices by proximity      |
| `webspider3d.weld`                     | `uv.weld`                  | Weld UVs at center                   |
| `webspider3d.merge_at_cursor`          | `uv.snap_selected`         | Merge UVs at cursor                  |
| `webspider3d.remove_doubles`           | `uv.remove_doubles`        | Merge UVs by distance                |
| `webspider3d.select_split`             | `uv.select_split`          | Split selected UVs                   |
| `webspider3d.pin`                      | `uv.pin`                   | Pin selected UVs                     |
| `webspider3d.unpin`                    | `uv.pin`                   | Unpin selected UVs                   |
| `webspider3d.invert_pin`               | `uv.pin`                   | Invert pin state of UVs              |
| `webspider3d.hide_selected`            | `uv.hide`                  | Hide selected UV faces                      |
| `webspider3d.reveal`                   | `uv.reveal`                | Reveal hidden UV faces                      |
| `webspider3d.hide_unselected`          | `uv.hide`                  | Hide unselected UV faces                    |
| `webspider3d.copy_uvs`                 | `uv.copy`                  | Copy selected UVs                          |
| `webspider3d.paste_uvs`                | `uv.paste`                 | Paste UVs                                   |
| `webspider3d.reset_uvs`                | `uv.reset`                 | Reset UVs to default 0-1 space              |
| `webspider3d.export_uv_layout`         | `uv.export_layout`         | Export UV layout with stored properties     |

### UV Functions Panel

The `WEBSPIDER_UV_PT_functions` panel provides essential UV editing operations organized in sections:
...
The `WEBSPIDER_UV_PT_tools` panel provides tool activation and properties for UV sculpting:
...
bpy.ops.webspider3d.activate_uv_sculpt_grab()

# Operator finds IMAGE_EDITOR and activates tool with context override
for area in context.screen.areas:
    if area.type == 'IMAGE_EDITOR' and area.spaces.active.mode == 'WEBSPIDER_UV':
...
# Must get tool from IMAGE_EDITOR context, not WEBSPIDER_UV_PROPERTIES
for area in context.screen.areas:
    if area.type == 'IMAGE_EDITOR' and area.spaces.active.mode == 'WEBSPIDER_UV':
        with context.temp_override(area=area):
            tool = ToolSelectPanelHelper.tool_active_from_context(context)
...
The C Transform panel (`WEBSPIDER_UV_PT_transform`) appears when the **Transform tool is active** in IMAGE_EDITOR and includes comprehensive transform controls:
...
```python
row.prop_enum(wm.webspider3d_uv_ui, "active_panel", 'NEW_PANEL', text="New", icon='ICON_NAME')
```

3. **Create panel class** in `panels.py`:

```python
class WEBSPIDER_UV_PT_new_panel(Panel):
    bl_label = "New Panel"
    bl_idname = "WEBSPIDER_UV_PT_new_panel"
    bl_space_type = 'WEBSPIDER_UV_PROPERTIES'
    bl_region_type = 'WINDOW'
    bl_options = set()
    ...
    def poll(cls, context):
        ...
        if not hasattr(wm, 'webspider3d_uv_ui'):
            return False
        return wm.webspider3d_uv_ui.active_panel == 'NEW_PANEL'
...
```python
class WEBSPIDER_UV_PT_my_tool_panel(Panel):
    bl_label = "My Tool"
    bl_idname = "WEBSPIDER_UV_PT_my_tool_panel"
    bl_space_type = 'WEBSPIDER_UV_PROPERTIES'
    bl_region_type = 'WINDOW'
    bl_options = set()
        ...
        for wm in bpy.data.window_managers:
            for window in wm.windows:
                for area in window.screen.areas:
                    if area.type == 'WEBSPIDER_UV_PROPERTIES':
...
def draw(self, context):
    ...
    row = col.row()
    row.scale_y = 1.3
    row.operator("webspider.some_operator", text="Execute")
```
...
class WEBSPIDER_OT_new_operation(Operator):
    """Description"""
    bl_idname = "webspider.new_operation"
    ...
    @classmethod
    def poll(cls, context):
        return poll_webspider3d_uv_edit_mode(context)  # Or poll_webspider3d_uv_mode()
...
class WEBSPIDER_OT_new_operation(Operator):
    @classmethod
    def poll(cls, context):
        # AVOID - use poll_webspider3d_uv_mode() instead
        for area in context.screen.areas:
            if area.type == 'IMAGE_EDITOR':
                sima = area.spaces.active
                if sima and sima.mode == 'WEBSPIDER_UV':
...
```cpp
static float webspider3d_new_property = 0.0f;
```

2. **Add event ID**:

```cpp
#define B_WEBSPIDER_UVEDIT_NEW 10
```

3. **Add handler** in `do_webspider3d_uvedit_transform()`:

```cpp
else if (event == B_WEBSPIDER_UVEDIT_NEW) {
    // Perform operation using webspider3d_new_property
}
```
...
```cpp
if (event != B_WEBSPIDER_UVEDIT_VERTEX && ... && event != B_WEBSPIDER_UVEDIT_NEW) {
    return;
}
```
...
```cpp
PanelLayout new_layout = panel->layout->panel(C, "WEBSPIDER_UV_new_panel", false);
...
if (new_layout.body) {
    ...
    uiBut *but = uiDefButF(block, ButType::Num, B_WEBSPIDER_UVEDIT_NEW,
                            IFACE_("Property:"),
                            0, y -= UI_UNIT_Y, 200, UI_UNIT_Y,
                            &webspider3d_new_property, 0.0f, 1.0f, "Tooltip");
}
```
...
| ---------------- | --------------- | ---------------------- |
| `WebSpider 3DUVUIState` | `properties.py` | UI state PropertyGroup |
| `WEBSPIDER_UV_PT_*`  | `panels.py`     | Panel classes          |
| `WEBSPIDER_OT_*`     | `operators.py`  | Operator wrappers      |

### Key C Functions

| Function                          | Purpose                                |
| --------------------------------- | -------------------------------------- |
| `find_image_editor_context()`     | Finds IMAGE_EDITOR in WEBSPIDER_UV mode    |
| `do_webspider3d_uvedit_transform()`     | Main callback for transform operations |
| `webspider3d_uvedit_center()`           | Calculate center of selected UVs       |
| `webspider3d_uvedit_translate()`        | Translate selected UVs                 |
| `webspider3d_uvedit_rotate()`           | Rotate selected UVs around center      |
| `webspider3d_uvedit_scale()`            | Scale selected UVs                     |
| `webspider3d_uvedit_bounds()`           | Get bounding box of selected UVs       |
| `webspider3d_uvedit_pivot_center()`     | Get center based on pivot mode         |
| `webspider3d_uv_transform_panel_draw()` | Draw Transform panel                   |
| `webspider3d_uv_redo_panel_draw()`      | Draw Redo panel                        |

### Key Constants

```cpp
// Event IDs for callback
#define B_WEBSPIDER_UVEDIT_VERTEX 4   // Translation
#define B_WEBSPIDER_UVEDIT_ROTATE 5   // Rotation
#define B_WEBSPIDER_UVEDIT_SCALE 6    // Scale
#define B_WEBSPIDER_UVEDIT_PIVOT 7    // Pivot change (no operation)
#define B_WEBSPIDER_UVEDIT_CURSOR 8   // 2D Cursor
...
| Path                                                     | Type             | Purpose             |
| -------------------------------------------------------- | ---------------- | ------------------- |
| `context.window_manager.webspider3d_uv_ui`                     | `WebSpider 3DUVUIState` | UI state            |
| `context.window_manager.webspider3d_uv_ui.active_panel`        | `EnumProperty`   | Current panel       |
...
class WEBSPIDER_UV_PT_my_panel(Panel):
    bl_idname = "WEBSPIDER_UV_PT_my_panel"
    bl_label = "My Panel"
    bl_space_type = 'WEBSPIDER_UV_PROPERTIES'
    bl_region_type = 'WINDOW'
    ...
    def poll(cls, context):
        wm = context.window_manager
        return hasattr(wm, 'webspider3d_uv_ui') and wm.webspider3d_uv_ui.active_panel == 'MY_PANEL'

    def draw(self, context):
        self.layout.operator("webspider.my_operator")
```
...
class WEBSPIDER_OT_my_op(Operator):
    bl_idname = "webspider.my_op"
