...
class WEBSPIDER_AI_OT_lookdev360_generate(Operator):
    """Generate PBR textures from selected objects using AI"""

    bl_idname = "webspider_ai.lookdev360_generate"
    ...
    def execute(self, context):
        ...
        if self.prompt and self.prompt.strip():
            ...
            prompt = getattr(scene, 'webspider_ai_lookdev360_prompt', '')
            reference_image = None
        elif props:
            prompt = props.prompt
            ...
            if not prompt or not prompt.strip():
                fallback = getattr(scene, 'webspider_ai_lookdev360_prompt', '')
        ...
        if hasattr(scene, 'webspider_ai_lookdev360_checkpoint'):
            scene.webspider_ai_lookdev360_checkpoint = checkpoint
        ...
        try:
            ...
            from webspider.modules.moodboard.core.lookdev360_queue import enqueue_lookdev360_job
...
classes = (
    WEBSPIDER_AI_OT_lookdev360_generate,
)
