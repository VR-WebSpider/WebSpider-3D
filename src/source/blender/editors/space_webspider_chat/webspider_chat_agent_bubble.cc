 ...
 *
 * Content is supplied by a Python menu type (WEBSPIDER_AI_CHAT_MT_agent_bubble)
...
static uiBlock *webspider_ai_chat_block_agent_bubble_create(bContext *C,
                                                     ARegion *region,
                                                     void * /*arg*/)
{
  ...
  MenuType *mt = WM_menutype_find("WEBSPIDER_AI_CHAT_MT_agent_bubble", true);
  if (mt) {
    ...
    layout.label("WebSpider 3D agent bubble loading…", ICON_INFO);
  }
...
static wmOperatorStatus webspider_ai_chat_agent_bubble_show_invoke(bContext *C,
                                                             wmOperator * /*op*/,
                                                             const wmEvent * /*event*/)
{
  UI_popup_block_invoke(C, webspider_ai_chat_block_agent_bubble_create, nullptr, nullptr);
...
void WEBSPIDER_AI_CHAT_OT_agent_bubble_show(wmOperatorType *ot)
{
   ...
   * auto-converts to the dotted Python form (webspider_ai_chat.agent_bubble_show)
  ...
  ot->idname = "WEBSPIDER_AI_CHAT_OT_agent_bubble_show";
  ot->description =
      "Open the floating agent bubble overlay that lets the user chat with the AI agent "
      "from anywhere in the WebSpider 3D interface";

  ot->invoke = webspider_ai_chat_agent_bubble_show_invoke;
